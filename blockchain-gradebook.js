  ...
  (async () => {
    ...
    try {
      await pool.query(`CREATE TABLE IF NOT EXISTS grade_attestations (
        ...
        comments TEXT,
        INDEX idx_tenant (tenant_id),
        INDEX idx_grade (tenant_id, grade_id),
        INDEX idx_attester (tenant_id, attester_id)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);
    ...
    try {
      await pool.query(`CREATE TABLE IF NOT EXISTS transcript_requests (
        ...
        purpose VARCHAR(200),
        status ENUM('pending','approved','sent','expired','rejected') DEFAULT 'pending',
