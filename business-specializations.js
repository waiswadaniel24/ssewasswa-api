...
`CREATE TABLE IF NOT EXISTS restaurant_order_items(id SERIAL PRIMARY KEY,tenant_id INTEGER REFERENCES tenants(id) ON DELETE CASCADE,order_id INTEGER REFERENCES restaurant_orders(id) ON DELETE CASCADE,item_name VARCHAR(255),quantity INTEGER NOT NULL,unit_price NUMERIC(10,2),status VARCHAR(20) DEFAULT 'pending')`,
`ALTER TABLE restaurant_order_items ADD COLUMN IF NOT EXISTS tenant_id INTEGER`,
// RETAIL
...
`CREATE TABLE IF NOT EXISTS retail_sale_items(id SERIAL PRIMARY KEY,tenant_id INTEGER REFERENCES tenants(id) ON DELETE CASCADE,sale_id INTEGER REFERENCES retail_sales(id) ON DELETE CASCADE,product_name VARCHAR(255),quantity INTEGER NOT NULL,unit_price NUMERIC(12,2))`,
`ALTER TABLE retail_sale_items ADD COLUMN IF NOT EXISTS tenant_id INTEGER`,
// SALON
...
`CREATE TABLE IF NOT EXISTS pharmacy_sale_items(id SERIAL PRIMARY KEY,tenant_id INTEGER REFERENCES tenants(id) ON DELETE CASCADE,sale_id INTEGER REFERENCES pharmacy_sales(id) ON DELETE CASCADE,drug_name VARCHAR(255),quantity INTEGER NOT NULL,unit_price NUMERIC(10,2),instructions TEXT)`,
...
`CREATE TABLE IF NOT EXISTS hardware_quotation_items(id SERIAL PRIMARY KEY,tenant_id INTEGER REFERENCES tenants(id) ON DELETE CASCADE,quotation_id INTEGER REFERENCES hardware_quotations(id) ON DELETE CASCADE,product_name VARCHAR(255),quantity NUMERIC(12,2),unit_price NUMERIC(12,2),total NUMERIC(12,2))`,
`ALTER TABLE hardware_quotation_items ADD COLUMN IF NOT EXISTS tenant_id INTEGER`,
// SUPERMARKET
