...
module.exports = function(app, pool, requireAuth, requireNotBanned, ah, esc, renderPage, audit, notify, sendEmail, sendSMS) {
  ...
  app.post('/api/crypto-wallets', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'crypto_wallets', r.rows[0].id); res.json(r.rows[0]);
  }));
  ...
  app.delete('/api/crypto-wallets/:id', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'delete', 'crypto_wallets', req.params.id); res.json({ ok: true });
  }));
  ...
  app.post('/api/crypto-donations', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'crypto_donations', r.rows[0].id); res.json(r.rows[0]);
  }));
  app.post('/api/crypto-donations/:id/confirm', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'update', 'crypto_donations', req.params.id); res.json(r.rows[0]);
  }));
  ...
  app.post('/api/inkind-categories', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'inkind_categories', r.rows[0].id); res.json(r.rows[0]);
  }));
  ...
  app.delete('/api/inkind-categories/:id', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'delete', 'inkind_categories', req.params.id); res.json({ ok: true });
  }));
  ...
  app.post('/api/inkind-donations', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'inkind_donations', r.rows[0].id); res.json(r.rows[0]);
  }));
  ...
  app.put('/api/inkind-donations/:id/acknowledge', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'update', 'inkind_donations', req.params.id); res.json(r.rows[0]);
  }));
  ...
  app.post('/api/planned-giving', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'planned_giving', r.rows[0].id); res.json(r.rows[0]);
  }));
  ...
  app.delete('/api/planned-giving/:id', requireAuth, ah(async (req, res) => { await pool.query('DELETE FROM planned_giving WHERE tenant_id=$1 AND id=$2', [req.session.user.tenant_id, req.params.id]); await audit(req, 'delete', 'planned_giving', req.params.id); res.json({ ok: true }); }));
  app.post('/api/planned-giving/:id/bequest', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'bequests', r.rows[0].id); res.json(r.rows[0]);
  }));
  ...
  app.put('/api/planned-giving/:id/realize', requireAuth, ah(async (req, res) => { const r = await pool.query('UPDATE planned_giving SET status=$1 WHERE tenant_id=$2 AND id=$3 RETURNING *', ['realized', req.session.user.tenant_id, req.params.id]); await audit(req, 'update', 'planned_giving', req.params.id); res.json(r.rows[0]); }));
  ...
  app.post('/api/board-members', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'board_members', r.rows[0].id); res.json(r.rows[0]);
  }));
  ...
  app.delete('/api/board-members/:id', requireAuth, ah(async (req, res) => { await pool.query('DELETE FROM board_members WHERE tenant_id=$1 AND id=$2', [req.session.user.tenant_id, req.params.id]); await audit(req, 'delete', 'board_members', req.params.id); res.json({ ok: true }); }));
  ...
  app.post('/api/board-giving', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'board_giving_pledges', r.rows[0].id); res.json(r.rows[0]);
  }));
  ...
  app.post('/api/board-giving/:id/pay', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'update', 'board_giving_pledges', req.params.id); res.json(r.rows[0]);
  }));
  ...
  app.post('/api/ticket-tiers', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'ticket_tiers', r.rows[0].id); res.json(r.rows[0]);
  }));
  ...
  app.delete('/api/ticket-tiers/:id', requireAuth, ah(async (req, res) => { await pool.query('DELETE FROM ticket_tiers WHERE tenant_id=$1 AND id=$2', [req.session.user.tenant_id, req.params.id]); await audit(req, 'delete', 'ticket_tiers', req.params.id); res.json({ ok: true }); }));
  ...
  app.post('/api/ticket-purchases/:id/checkin', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'update', 'ticket_purchases', req.params.id); res.json(r.rows[0]);
  }));
  ...
  app.post('/api/auction-items', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'auction_items', r.rows[0].id); res.json(r.rows[0]);
  }));
  ...
  app.delete('/api/auction-items/:id', requireAuth, ah(async (req, res) => { await pool.query('DELETE FROM auction_items WHERE tenant_id=$1 AND id=$2', [req.session.user.tenant_id, req.params.id]); await audit(req, 'delete', 'auction_items', req.params.id); res.json({ ok: true }); }));
  ...
  app.post('/api/auction-items/:id/close', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'update', 'auction_items', req.params.id); res.json(r.rows[0]);
  }));
  ...
  app.post('/api/sponsorship-packages', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'sponsorship_packages', r.rows[0].id); res.json(r.rows[0]);
  }));
  ...
  app.delete('/api/sponsorship-packages/:id', requireAuth, ah(async (req, res) => { await pool.query('DELETE FROM sponsorship_packages WHERE tenant_id=$1 AND id=$2', [req.session.user.tenant_id, req.params.id]); await audit(req, 'delete', 'sponsorship_packages', req.params.id); res.json({ ok: true }); }));
  ...
  app.post('/api/daf', requireAuth, ah(async (req, res) => {
    ...
    await audit(req, 'create', 'donor_advised_funds', r.rows[0].id); res.json(r.rows[0]);
  }));
