  ...
  (async function () {
    try {
      await pool.query(`CREATE TABLE IF NOT EXISTS green_initiatives (
        ...
        description TEXT,
        category ENUM('energy','waste','water','biodiversity','transport','education','community') DEFAULT 'energy',
      ...
      await pool.query(`CREATE TABLE IF NOT EXISTS energy_readings (
        ...
        reading_type ENUM('electricity','gas','solar_generation','solar_export') DEFAULT 'electricity',
        ...
        created_at DATETIME DEFAULT NOW(),
        INDEX idx_tenant (tenant_id),
        INDEX idx_meter_date (tenant_id, meter_id, recorded_at),
        INDEX idx_recorded (tenant_id, recorded_at)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);

      await pool.query(`CREATE TABLE IF NOT EXISTS waste_records (
        id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        tenant_id BIGINT UNSIGNED NOT NULL,
        waste_type ENUM('recyclable','organic','general','hazardous','e_waste','construction') DEFAULT 'general',
        weight_kg DECIMAL(10,2) NOT NULL,
        disposal_method ENUM('recycled','composted','landfill','incinerated','specialized') DEFAULT 'landfill',
        recorded_at DATETIME NOT NULL,
        ...
        created_at DATETIME DEFAULT NOW(),
        INDEX idx_tenant (tenant_id),
        INDEX idx_date (tenant_id, recorded_at)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);
      ...
      await pool.query(`CREATE TABLE IF NOT EXISTS tree_planting (
        ...
        planted_by BIGINT UNSIGNED,
        planted_at DATE NOT NULL,
        status ENUM('planned','planted','growing','mature','diseased','removed') DEFAULT 'planned',
      ...
      await pool.query(`CREATE TABLE IF NOT EXISTS sustainability_goals (
        ...
        title VARCHAR(255) NOT NULL,
        description TEXT,
        category ENUM('energy','waste','water','carbon','biodiversity','education') DEFAULT 'energy',
      ...
      await pool.query(`CREATE TABLE IF NOT EXISTS green_certifications (
        ...
        achieved_date DATE,
        expiry_date DATE,
        status ENUM('pending','achieved','expired','renewed') DEFAULT 'pending',
      ...
      await pool.query(`CREATE TABLE IF NOT EXISTS eco_clubs (
        ...
        teacher_advisor_id BIGINT UNSIGNED,
