...
(async () => {
  try {
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_investments (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, type TEXT, buy_price NUMERIC(15,2), current_value NUMERIC(15,2), quantity NUMERIC(15,4), purchase_date DATE, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_savings_goals (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, target_amount NUMERIC(15,2), current_amount NUMERIC(15,2) DEFAULT 0, deadline DATE, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_savings_deposits (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, goal_id INTEGER REFERENCES ind_savings_goals(id) ON DELETE CASCADE, amount NUMERIC(15,2), note TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_loans (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, lender TEXT, principal NUMERIC(15,2), interest_rate NUMERIC(5,2), outstanding NUMERIC(15,2), emi_amount NUMERIC(15,2), start_date DATE, end_date DATE, status TEXT DEFAULT 'active', notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_loan_payments (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, loan_id INTEGER REFERENCES ind_loans(id) ON DELETE CASCADE, amount NUMERIC(15,2), payment_date DATE DEFAULT CURRENT_DATE, note TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_expenses (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, category TEXT, amount NUMERIC(15,2), description TEXT, expense_date DATE DEFAULT CURRENT_DATE, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_networth_snapshots (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, total_assets NUMERIC(15,2), total_liabilities NUMERIC(15,2), net_worth NUMERIC(15,2), notes TEXT, snapshot_date DATE DEFAULT CURRENT_DATE, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_recurring_txns (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, amount NUMERIC(15,2), type TEXT DEFAULT 'expense', category TEXT, frequency TEXT DEFAULT 'monthly', next_due DATE, is_active BOOLEAN DEFAULT true, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_habits (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, frequency TEXT DEFAULT 'daily', color TEXT DEFAULT '#ec4899', target_count INTEGER DEFAULT 1, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_habit_checkins (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, habit_id INTEGER REFERENCES ind_habits(id) ON DELETE CASCADE, checkin_date DATE DEFAULT CURRENT_DATE, note TEXT, created_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(habit_id, checkin_date))`);
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_time_logs (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, project TEXT, activity TEXT, start_time TIMESTAMPTZ, end_time TIMESTAMPTZ, duration_minutes INTEGER, note TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_pomodoro_sessions (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, task TEXT, duration_minutes INTEGER DEFAULT 25, type TEXT DEFAULT 'focus', started_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_calendar_events (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, title TEXT NOT NULL, description TEXT, event_date DATE, event_time TIME, category TEXT DEFAULT 'personal', location TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_journal_entries (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, title TEXT, content TEXT, mood TEXT, tags TEXT, entry_date DATE DEFAULT CURRENT_DATE, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_focus_sessions (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, goal TEXT, started_at TIMESTAMPTZ DEFAULT NOW(), ended_at TIMESTAMPTZ, duration_minutes INTEGER, completed BOOLEAN DEFAULT false, note TEXT)`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_health_metrics (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, metric_type TEXT, value NUMERIC(10,2), unit TEXT, recorded_date DATE DEFAULT CURRENT_DATE, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_medications (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, dosage TEXT, frequency TEXT, start_date DATE, end_date DATE, is_active BOOLEAN DEFAULT true, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_medication_log (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, medication_id INTEGER REFERENCES ind_medications(id) ON DELETE CASCADE, taken_at TIMESTAMPTZ DEFAULT NOW(), status TEXT DEFAULT 'taken')`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_workouts (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, type TEXT, duration_minutes INTEGER, calories INTEGER, sets INTEGER, reps INTEGER, notes TEXT, workout_date DATE DEFAULT CURRENT_DATE, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_water_intake (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, glasses INTEGER DEFAULT 0, intake_date DATE DEFAULT CURRENT_DATE, goal INTEGER DEFAULT 8, created_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(tenant_id, user_email, intake_date))`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_sleep_log (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, bedtime TIME, wake_time TIME, quality INTEGER DEFAULT 5, notes TEXT, sleep_date DATE DEFAULT CURRENT_DATE, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_wellness_checkins (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, mood TEXT, stress_level INTEGER DEFAULT 5, gratitude TEXT, energy_level INTEGER DEFAULT 5, checkin_date DATE DEFAULT CURRENT_DATE, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_books (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, title TEXT NOT NULL, author TEXT, genre TEXT, status TEXT DEFAULT 'want', rating INTEGER, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_skills (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, category TEXT, level TEXT DEFAULT 'beginner', progress INTEGER DEFAULT 0, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_courses (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, title TEXT NOT NULL, provider TEXT, category TEXT, progress INTEGER DEFAULT 0, status TEXT DEFAULT 'in_progress', certificate TEXT, deadline DATE, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_bookmarks (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, title TEXT NOT NULL, url TEXT, category TEXT, tags TEXT, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_flashcard_decks (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, description TEXT, category TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_flashcards (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, deck_id INTEGER REFERENCES ind_flashcard_decks(id) ON DELETE CASCADE, front TEXT NOT NULL, back TEXT NOT NULL, last_studied TIMESTAMPTZ, confidence INTEGER DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_contacts (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, phone TEXT, email TEXT, company TEXT, category TEXT, address TEXT, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_wiki_pages (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, title TEXT NOT NULL, content TEXT, category TEXT, parent_id INTEGER REFERENCES ind_wiki_pages(id), created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_trips (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, destination TEXT, start_date DATE, end_date DATE, budget NUMERIC(15,2), status TEXT DEFAULT 'planning', notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_trip_items (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, trip_id INTEGER REFERENCES ind_trips(id) ON DELETE CASCADE, item_type TEXT, name TEXT, date DATE, time TIME, cost NUMERIC(15,2), notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_recipes (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, title TEXT NOT NULL, ingredients TEXT, steps TEXT, cook_time INTEGER, servings INTEGER, category TEXT, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_wishlist_items (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, estimated_cost NUMERIC(15,2), priority TEXT DEFAULT 'medium', url TEXT, purchased BOOLEAN DEFAULT false, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_subscriptions (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, cost NUMERIC(15,2), billing_cycle TEXT DEFAULT 'monthly', next_billing DATE, category TEXT, is_active BOOLEAN DEFAULT true, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_gifts (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, recipient TEXT, occasion TEXT, direction TEXT DEFAULT 'given', cost NUMERIC(15,2), date_given DATE, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ind_bucket_list (id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE, user_email TEXT NOT NULL, name TEXT NOT NULL, category TEXT DEFAULT 'personal', completed BOOLEAN DEFAULT false, completed_at DATE, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
