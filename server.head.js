...
if (!process.env.DATABASE_URL) console.error('WARNING: DATABASE_URL not set!');
...
app.get('/', ah(async (req, res) => {
  ...
  res.send(renderPage('SSEWASSWA', `<div class="hero"><h1>Learn - Shop - Play - Earn</h1><div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap"><a href="/signup" class="btn btn-green" style="font-size:18px;padding:16px 32px">Get Started</a><a href="/demo" class="btn btn-gold" style="font-size:18px;padding:16px 32px">Book Demo</a></div></div><div class="stats"><div class="stat-card"><div class="stat-num">50K+</div><div>Users</div></div><div class="stat-card"><div class="stat-num">500+</div><div>Schools</div></div><div class="stat-card"><div class="stat-num">10M+</div><div>Rewards</div></div></div><div class="grid"><div class="card" style="text-align:center;cursor:pointer" onclick="location.href='/learning'"><div style="font-size:48px">≡ƒôÜ</div><h3>Learning</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href='/store'"><div style="font-size:48px">≡ƒ¢Æ</div><h3>Store</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href='/marketplace'"><div style="font-size:48px">≡ƒÅ¬</div><h3>Marketplace</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href='/videos'"><div style="font-size:48px">≡ƒÄ¼</div><h3>Videos</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href='/games'"><div style="font-size:48px">≡ƒÄ«</div><h3>Games</h3></div></div><div class="card"><h2>Latest News</h2><div class="grid">${news}</div></div>`, null, true));
}));

app.get('/login', (req, res) => {
  res.send(renderPage('Login', `<div class="card" style="max-width:450px;margin:40px auto"><div style="text-align:center;margin-bottom:24px"><div style="font-size:60px">≡ƒÄô</div><h1>Welcome Back</h1></div><form method="POST" action="/login"><input name="email" placeholder="Email" type="email" required><input name="password" placeholder="Password" type="password" required><button type="submit" class="btn" style="width:100%;font-size:18px;padding:16px">Login</button></form><div style="text-align:center;margin-top:20px"><a href="/signup" style="color:#1e40af">Create Account</a></div></div>`, null, true));
});
...
app.get('/parent/dashboard', checkDb, ah(async (req, res) => {
  ...
  const c = s.rows.map(x => `<div class="card"><h3>${esc(x.name)}</h3><p>Class: ${esc(x.class||'N/A')}</p><p>Balance: <strong class="badge badge-red">UGX ${x.balance}</strong></p><div style="margin-top:12px"><a href="/parent/pay/${x.id}" class="btn btn-green">Pay</a></div></div>`).join('');
...
app.get('/bonus', requireAuth, checkDb, ah(async (req, res) => {
  ...
  res.send(renderPage('Rewards', `<div class="hero" style="padding:40px 20px"><h2>My Wallet</h2><div class="stat-num" style="font-size:48px;color:white;-webkit-text-fill-color:white">UGX ${w.balance}</div><div style="display:flex;gap:12px;justify-content:center;margin-top:20px"><a href="/bonus/withdraw" class="btn btn-green">Withdraw</a><a href="/bonus/affiliate" class="btn btn-purple">Affiliate</a></div></div><div class="card"><h3>Recent</h3><table><thead><tr><th>Date</th><th>Type</th><th>Amount</th></tr></thead><tbody>${rows||'<tr><td colspan="3">None yet</td></tr>'}</tbody></table></div>`, {tenant_name:req.tenant.name}, false, req.lang));
}));
...
app.get('/games/play/:id', requireAuth, checkDb, (req, res) => {
  res.send(renderPage('Quiz', `<div class="card" style="max-width:600px;margin:40px auto"><h1>Math Quiz</h1><div id="qa" style="text-align:center;margin:20px 0"><div style="font-size:36px" id="q"></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:20px"><button class="btn" onclick="ck(this)" id="a1"></button><button class="btn" onclick="ck(this)" id="a2"></button><button class="btn" onclick="ck(this)" id="a3"></button><button class="btn" onclick="ck(this)" id="a4"></button></div></div><div id="r" style="display:none;text-align:center"><h2>Done!</h2><p class="badge badge-green" style="font-size:18px">+30 UGX</p><a href="/games" class="btn" style="margin-top:20px">Back</a></div></div><script>let qs=[],i=0;for(let j=0;j<5;j++){let a=Math.floor(Math.random()*20)+1,b=Math.floor(Math.random()*20)+1,o=["+","-","x"][Math.floor(Math.random()*3)],ans=o==="+"?a+b:o==="-"?a-b:a*b;qs.push({q:a+" "+o+" "+b+" = ?",ans});}function sq(){if(i>=5){document.getElementById("qa").style.display="none";document.getElementById("r").style.display="block";fetch("/bonus/claim/game/quiz").catch(function(){});return;}let c=qs[i];document.getElementById("q").textContent=c.q;let opts=[c.ans];while(opts.length<4){let w=c.ans+Math.floor(Math.random()*20)-10;if(!opts.includes(w))opts.push(w);}opts.sort(function(){return Math.random()-0.5;});for(let j=1;j<=4;j++){document.getElementById("a"+j).textContent=opts[j-1];}i++;}function ck(b){sq();}sq();</script>`, {tenant_name:req.tenant.name}, false, req.lang));
}));
...
app.get('/app/students', requireAuth, checkDb, ah(async (req, res) => {
  ...
  const t = rows.map(s => `<tr><td>${esc(s.name)}</td><td>${esc(s.class||'-')}</td><td>${esc(s.guardian_phone||'-')}</td><td>UGX ${s.balance}</td><td><a href="/app/students/report/${s.id}" class="btn" style="font-size:12px;padding:8px">Report</a> <a href="/app/students/edit/${s.id}" class="btn btn-orange" style="font-size:12px;padding:8px">Edit</a></td></tr>`).join('');
...
app.get('/app/students/report/:id', requireAuth, checkDb, ah(async (req, res) => {
  ...
  const gRows = g.rows.map(x => `<tr><td>${esc(x.subject)}</td><td>${x.score}</td><td>${esc(x.term||'-')}</td><td>${x.year||'-'}</td></tr>`).join('');
  const fRows = f.rows.map(x => `<tr><td>${esc(x.term||'-')} ${x.year||''}</td><td>${x.amount}</td><td>${x.paid}</td><td>${x.amount-x.paid}</td></tr>`).join('');
  res.send(renderPage('Report', `<div class="card" style="text-align:center"><h1>${esc(req.tenant.name)}</h1><h2>REPORT CARD</h2><p><strong>Name:</strong> ${esc(s.name)} | <strong>Class:</strong> ${esc(s.class||'N/A')} | <strong>Balance:</strong> UGX ${s.balance}</p><p><strong>Attendance:</strong> ${pct}% (${a.rows[0].p}/${a.rows[0].t})</p></div><div class="card"><h3>Academics</h3><table><thead><tr><th>Subject</th><th>Score</th><th>Term</th><th>Year</th></tr></thead><tbody>${gRows||'<tr><td colspan="4">No grades</td></tr>'}</tbody></table></div><div class="card"><h3>Fees</h3><table><thead><tr><th>Term</th><th>Due</th><th>Paid</th><th>Balance</th></tr></thead><tbody>${fRows||'<tr><td colspan="4">No fees</td></tr>'}</tbody></table></div><div class="card" style="text-align:center"><button onclick="window.print()" class="btn btn-green">Print</button></div>`, {tenant_name:req.tenant.name}, false, req.lang));
}));
...
app.get('/super-admin', requireAuth, requireRole('super_admin'), checkDb, ah(async (req, res) => {
  ...
  res.send(renderPage('Super Admin', `<div class="hero" style="background:linear-gradient(135deg,#dc2626,#ef4444);padding:30px"><h1>Platform Control</h1></div><div class="stats"><div class="stat-card"><div class="stat-num">UGX ${r30.toLocaleString()}</div><div>30 Days</div></div><div class="stat-card"><div class="stat-num">UGX ${rt.toLocaleString()}</div><div>Total</div></div><div class="stat-card"><div class="stat-num">UGX ${w.toLocaleString()}</div><div>Wallet</div></div><div class="stat-card"><div class="stat-num">${sc}</div><div>Schools</div></div><div class="stat-card"><div class="stat-num">${u}</div><div>Users</div></div></div><div class="card"><h3>Actions</h3><div style="display:flex;flex-direction:column;gap:8px;margin-top:12px"><a href="/demo" class="btn btn-green">Create School</a><a href="/super-admin/payout-developer" class="btn btn-red">Payout</a></div></div>`, {tenant_name:req.tenant.name}));
}));
...
async function initDB() {
  for (let attempt = 1; attempt <= 5; attempt++) {
    ...
    try {
      ...
      if (!process.env.DATABASE_URL) { console.error('DATABASE_URL not set!'); return false; }
      client = await pool.connect();
      if (!client) throw new Error('null client');
      ...
      if (attempt === 5) { console.error('All attempts failed'); return false; }
      await sleep(5000);
    } finally {
