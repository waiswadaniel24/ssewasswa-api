...
async function checkDb(req, res, next) {
  if (!dbReady) {
    return res.status(503).send(`<div style="text-align:center;padding:50px;font-family:sans-serif"><h1>⏳ Waking up database...</h1><p>This takes 10-20 seconds on free tiers. Please wait and <a href="/">refresh</a>.</p></div>`);
  }
  next();
}
...
async function sendSMS(phone, message) {
  if (SMS_CONFIG.apiKey === 'demo') return;
  try {
...
app.get('/parent/login', (req, res) => {
  res.send(renderPage('Parent Login', '<div class="card" style="max-width:400px;margin:60px auto"><h1>Parent Login</h1><p>Enter phone to receive OTP</p><form method="POST" action="/parent/send-otp"><input name="phone" placeholder="07XXXXXXXX" required /><button type="submit" class="btn" style="width:100%">Send OTP</button></form></div>'));
});
...
app.post('/parent/verify-otp', checkDb, async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT * FROM parent_otps WHERE phone=$1 AND otp=$2 AND expires_at > NOW() AND used=false ORDER BY id DESC LIMIT 1', [phone, otp]);
...
app.get('/parent/dashboard', checkDb, async (req, res) => {
  ...
  try {
    ...
    const cards = students.rows.map(s => `<div class="card"><h3>${esc(s.name)}</h3><p>Class: ${esc(s.class)||'-'}</p><p>Balance: UGX ${s.balance}</p><a href="/parent/pay/${s.id}" class="btn btn-green">Pay Fees</a></div>`).join('');
...
app.get('/super-admin/tenants', requireAuth, requireRole('super_admin'), checkDb, async (req, res) => {
  ...
  const t = rows.map(r => `<tr><td>${esc(r.name)}</td><td>${esc(r.subdomain)}</td><td>${esc(r.plan)}</td></tr>`).join('');
  res.send(renderPage('Schools', `<div class="card"><table><thead><tr><th>Name</th><th>Sub</th><th>Plan</th></tr></thead><tbody>${t}</tbody></table></div>`));
});
...
app.get('/super-admin/users', requireAuth, requireRole('super_admin'), checkDb, async (req, res) => {
  ...
  const t = rows.map(r => `<tr><td>${esc(r.email)}</td><td>${esc(r.role)}</td><td>${esc(r.school)}</td></tr>`).join('');
  res.send(renderPage('Users', `<div class="card"><table><thead><tr><th>Email</th><th>Role</th><th>School</th></tr></thead><tbody>${t}</tbody></table></div>`));
});
...
app.post('/app/students/add', requireAuth, checkDb, async (req, res) => {
  try {
    ...
    if (guardian_phone) await sendSMS(guardian_phone, `${name} registered.`);
...
app.get('/app/fees', requireAuth, checkDb, async (req, res) => {
  ...
  res.send(renderPage('Fees', `<div class="card"><h1>Fees</h1><a href="/app/fees/add" class="btn btn-green">Record</a><table style="margin-top:16px"><thead><tr><th>Student</th><th>Due</th><th>Paid</th><th>Term</th><th>Method</th></tr></thead><tbody>${t||'<tr><td colspan="5">None</td></tr>'}</tbody></table></div>`, { tenant_name: req.tenant.name }));
});

app.get('/app/fees/add', requireAuth, checkDb, async (req, res) => {
  const students = await pool.query('SELECT id, name FROM students WHERE tenant_id=$1', [req.tenantId]);
  const o = students.rows.map(s => `<option value="${s.id}" ${req.query.student_id==s.id?'selected':''}>${esc(s.name)}</option>`).join('');
  res.send(renderPage('Pay', `<div class="card" style="max-width:500px"><h1>Record Payment</h1><form method="POST" action="/app/fees/add"><select name="student_id" required><option value="">Select</option>${o}</select><input name="amount" type="number" required><input name="paid" type="number" required><input name="term"><input name="year" type="number" value="${new Date().getFullYear()}"><select name="payment_method"><option>Cash</option><option>MoMo</option></select><button class="btn btn-green" style="width:100%">Save</button></form></div>`, { tenant_name: req.tenant.name }));
});

app.post('/app/fees/add', requireAuth, checkDb, async (req, res) => {
  try {
...
app.get('/app/attendance/mark', requireAuth, checkDb, async (req, res) => {
  const students = await pool.query('SELECT id, name FROM students WHERE tenant_id=$1', [req.tenantId]);
  const b = students.rows.map(s => `<label style="display:block;margin:8px 0"><input type="checkbox" name="p_${s.id}" checked> ${esc(s.name)}</label>`).join('');
  res.send(renderPage('Mark', `<div class="card" style="max-width:500px"><h1>Mark Attendance</h1><form method="POST" action="/app/attendance/mark">${b}<button class="btn btn-green" style="width:100%;margin-top:16px">Save</button></form></div>`, { tenant_name: req.tenant.name }));
});

app.post('/app/attendance/mark', requireAuth, checkDb, async (req, res) => {
  try {
...
app.get('/app/grades', requireAuth, checkDb, async (req, res) => {
  ...
  res.send(renderPage('Grades', `<div class="card"><h1>Grades</h1><a href="/app/grades/add" class="btn btn-green">Add</a><table style="margin-top:16px"><thead><tr><th>Student</th><th>Subject</th><th>Score</th><th>Term</th></tr></thead><tbody>${t||'<tr><td colspan="4">None</td></tr>'}</tbody></table></div>`, { tenant_name: req.tenant.name }));
});

app.get('/app/grades/add', requireAuth, checkDb, async (req, res) => {
  const students = await pool.query('SELECT id, name FROM students WHERE tenant_id=$1', [req.tenantId]);
  const o = students.rows.map(s => `<option value="${s.id}">${esc(s.name)}</option>`).join('');
...
app.post('/app/grades/add', requireAuth, checkDb, async (req, res) => {
  try {
...
// --- BACKGROUND DB INIT & SERVER START ---
