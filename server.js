...
app.use(session({
  ...
  secret: process.env.SESSION_SECRET || 'ssewasswa-secret-' + crypto.randomBytes(16).toString('hex'),
  resave: false,
...
function isValidPhone(phone) { return /^(\+?256|0)[7]\d{8}$/.test((phone || '').replace(/\s/g, '')); }
...
function renderPage(title, content, user, isPublic, lang) {
  ...
  let nav = '';
  if (user && !isPublic) {
    nav = '<div style="background:linear-gradient(135deg,#1e40af,#3b82f6);color:white;padding:10px 20px;display:flex;justify-content:space-between;align-items:center;margin:0 0 24px 0;flex-wrap:wrap"><div style="font-weight:700;font-size:18px">' + esc(user.tenant_name || 'SSEWASSWA') + '</div><div style="display:flex;gap:6px;flex-wrap:wrap;font-size:12px"><a href="/" style="color:white;text-decoration:none">🏠</a><a href="/app" style="color:white;text-decoration:none">📊</a><a href="/app/students" style="color:white;text-decoration:none">🎓</a><a href="/app/fees" style="color:white;text-decoration:none">💰</a><a href="/app/attendance" style="color:white;text-decoration:none">✅</a><a href="/app/grades" style="color:white;text-decoration:none">📝</a><a href="/app/analytics" style="color:white;text-decoration:none">📈</a><a href="/learning" style="color:white;text-decoration:none">📚</a><a href="/learning/live" style="color:white;text-decoration:none">🎥</a><a href="/store" style="color:white;text-decoration:none">🛒</a><a href="/marketplace" style="color:white;text-decoration:none">🏪</a><a href="/videos" style="color:white;text-decoration:none">🎬</a><a href="/games" style="color:white;text-decoration:none">🎮</a><a href="/news" style="color:white;text-decoration:none">📰</a><a href="/bonus" style="color:white;text-decoration:none">🎁</a><a href="/app/referrals" style="color:white;text-decoration:none">💰</a><a href="/premium" style="color:white;text-decoration:none">⭐</a><a href="/app/chatbot" style="color:white;text-decoration:none">🤖</a><a href="/app/settings" style="color:white;text-decoration:none">⚙️</a><a href="/logout" style="color:white;text-decoration:none">🚪</a></div></div>';
  } else if (isPublic) {
...
const requireAuth = (req, res, next) => { if (!req.session || !req.session.user) return res.redirect('/login'); req.tenant = req.session.tenant; req.tenantId = req.session.tenant ? req.session.tenant.id : null; req.lang = req.query.lang || detectLang(req); next(); };
const requireRole = (role) => (req, res, next) => { if (!req.session || !req.session.user || req.session.user.role !== role) return res.status(403).send(renderPage('Forbidden', '<div class="card"><h1>403</h1></div>', { tenant_name: req.tenant ? req.tenant.name : null })); next(); };
const requireStaff = (req, res, next) => { if (!req.session || !req.session.user || !['admin', 'super_admin', 'teacher'].includes(req.session.user.role)) return res.status(403).send(renderPage('Forbidden', '<div class="card"><h1>403 Staff Only</h1></div>', { tenant_name: req.tenant ? req.tenant.name : null })); next(); };
const requireAdmin = (req, res, next) => { if (!req.session || !req.session.user || !['admin', 'super_admin'].includes(req.session.user.role)) return res.status(403).send(renderPage('Forbidden', '<div class="card"><h1>403 Admin Only</h1></div>', { tenant_name: req.tenant ? req.tenant.name : null })); next(); };

async function sendSMS(phone, message) {
  if (!isValidPhone(phone)) return;
  if (SMS_CONFIG.apiKey === 'demo') { console.log('[SMS DEMO] ' + phone + ': ' + message); return; }
  try { await axios.post('https://api.africastalking.com/version1/messaging', 'username=' + SMS_CONFIG.username + '&to=' + phone + '&message=' + encodeURIComponent(message) + '&from=' + SMS_CONFIG.senderId, { headers: { 'apiKey': SMS_CONFIG.apiKey, 'Content-Type': 'application/x-www-form-urlencoded' } }); } catch (e) { console.error('SMS Error:', e.message); }
}

async function sendWhatsApp(phone, message) {
  if (!isValidPhone(phone)) return;
  if (WHATSAPP_CONFIG.token === 'demo') { console.log('[WA DEMO] ' + phone + ': ' + message); return; }
  try { await axios.post('https://graph.facebook.com/v18.0/' + WHATSAPP_CONFIG.phoneId + '/messages', { messaging_product: 'whatsapp', to: phone, text: { body: message } }, { headers: { 'Authorization': 'Bearer ' + WHATSAPP_CONFIG.token } }); } catch (e) { console.error('WA Error:', e.message); }
}
...
async function sendPushToUser(email, title, body) {
  try {
    ...
    for (const sub of subs.rows) {
      try { await webpush.sendNotification({ endpoint: sub.endpoint, keys: sub.keys }, JSON.stringify({ title, body })); } catch (e) { console.error('Push failed:', e.message); }
    }
...
app.post('/api/subscribe', requireAuth, checkDb, asyncHandler(async (req, res) => {
  if (!req.body || !req.body.endpoint || !req.body.keys) return res.status(400).json({ error: 'Missing data' });
...
app.post('/api/check-teacher-milestone', requireAuth, checkDb, asyncHandler(async (req, res) => {
  ...
  const alreadyPaid = (await pool.query("SELECT id FROM bonus_earnings WHERE user_id=$1 AND type='teacher_milestone'", [req.session.user.email])).rows[0];
  if (studentCount >= 10 && !alreadyPaid) {
    ...
    await sendPushToUser(req.session.user.email, '🎉 Bonus!', 'You earned UGX 5,000 for adding 10 students!');
    const referrer = await pool.query('SELECT referrer_email FROM referral_stats WHERE referred_email=$1', [req.session.user.email]);
    if (referrer.rows[0] && referrer.rows[0].referrer_email) await addBonus(referrer.rows[0].referrer_email, req.tenantId, 2000, 'referral_bonus', 'Your referral hit 10 students');
...
app.get('/share/:platform', requireAuth, checkDb, asyncHandler(async (req, res) => {
  ...
  const link = 'https://' + req.headers.host + '/signup?ref=' + encodeURIComponent(req.session.user.email);
  await pool.query('INSERT INTO viral_shares (user_email, platform, link_shared) VALUES ($1,$2,$3)', [req.session.user.email, platform, link]);
  const messages = { whatsapp: 'https://wa.me/?text=' + encodeURIComponent('Join SSEWASSWA - Free School Management + Earn Rewards! ' + link), facebook: 'https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent(link), sms: 'sms:?body=' + encodeURIComponent('Join SSEWASSWA: ' + link) };
...
app.get('/', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    newsCards = feed.items.slice(0, 6).map(i => '<div class="card"><h4>' + esc(i.title) + '</h4><p style="color:#64748b;font-size:14px">' + esc((i.contentSnippet || '').substring(0, 100)) + '...</p><a href="/bonus/claim/news?url=' + encodeURIComponent(i.link) + '" class="btn btn-orange" style="font-size:12px;padding:8px 16px" target="_blank">Read +20</a></div>').join('');
  } catch (e) { console.error('RSS error:', e.message); }
  res.send(renderPage('SSEWASSWA', '<div class="hero"><h1>Learn - Shop - Play - Earn</h1><p>Your all-in-one platform.</p><div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap"><a href="/signup" class="btn btn-green" style="font-size:18px;padding:16px 32px">Get Started</a><a href="/demo" class="btn btn-gold" style="font-size:18px;padding:16px 32px">📞 Book Demo</a></div></div><div class="stats"><div class="stat-card"><div class="stat-num">50K+</div><div>Users</div></div><div class="stat-card"><div class="stat-num">500+</div><div>Schools</div></div><div class="stat-card"><div class="stat-num">10M+</div><div>Rewards</div></div></div><div class="grid"><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/learning\'"><div style="font-size:48px;margin-bottom:12px">📚</div><h3>Learning</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/store\'"><div style="font-size:48px;margin-bottom:12px">🛒</div><h3>Store</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/marketplace\'"><div style="font-size:48px;margin-bottom:12px">🏪</div><h3>Marketplace</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/videos\'"><div style="font-size:48px;margin-bottom:12px">🎬</div><h3>Videos</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/games\'"><div style="font-size:48px;margin-bottom:12px">🎮</div><h3>Games</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/learning/live\'"><div style="font-size:48px;margin-bottom:12px">🎥</div><h3>Live Classes</h3></div></div><div class="card" style="margin-top:20px"><h2>Latest News</h2><div class="grid">' + newsCards + '</div></div>', null, true));
}));
...
app.post('/login', checkDb, asyncHandler(async (req, res) => {
  ...
  if (!user.rows[0] || !(await bcrypt.compare(req.body.password, user.rows[0].password_hash))) return res.status(401).send(renderPage('Failed', '<div class="card" style="text-align:center"><h1>Invalid Credentials</h1><a href="/login" class="btn">Try Again</a></div>', null, true));
...
app.post('/signup', checkDb, asyncHandler(async (req, res) => {
  try {
    ...
    await pool.query('INSERT INTO users (tenant_id, email, password_hash, role, full_name, phone, approved) VALUES ($1,$2,$3,$4,$5,$6,$7)', [tenantId, req.body.email, await bcrypt.hash(req.body.password, 10), req.body.role, req.body.full_name, req.body.phone, true]);
    await pool.query('INSERT INTO wallets (tenant_id, user_email, balance) VALUES ($1,$2,0)', [tenantId, req.body.email]);
    await addBonus(req.body.email, tenantId, 100, 'signup', 'Welcome bonus');
    if (req.body.ref) {
      await addBonus(req.body.ref, tenantId, 200, 'referral', 'Referred ' + req.body.email);
...
app.post('/parent/send-otp', checkDb, asyncHandler(async (req, res) => {
...
app.post('/parent/verify-otp', checkDb, asyncHandler(async (req, res) => {
  ...
  if (!p.rows[0]) {
    ...
    await pool.query('INSERT INTO parents (phone, verified, tenant_id) VALUES ($1,true,$2)', [req.body.phone, t.rows[0] ? t.rows[0].id : 1]);
...
app.get('/parent/dashboard', checkDb, asyncHandler(async (req, res) => {
  ...
  const c = s.rows.map(x => '<div class="card"><h3>' + esc(x.name) + '</h3><p>Class: ' + esc(x.class || 'N/A') + '</p><p>Balance: <strong class="badge badge-red">UGX ' + x.balance + '</strong></p><div style="margin-top:12px"><a href="/parent/pay/' + x.id + '" class="btn btn-green">Pay Fees</a><a href="/app/students/report/' + x.id + '" class="btn" target="_blank">Report</a></div></div>').join('');
...
app.get('/parent/referrals', checkDb, asyncHandler(async (req, res) => {
  if (!req.session.parent) return res.redirect('/parent/login');
  const refs = await pool.query('SELECT p.phone, p.name, COUNT(s.id) as children FROM parents p LEFT JOIN students s ON p.id=s.parent_id WHERE p.referred_by=$1 GROUP BY p.id, p.phone, p.name', [req.session.parent.phone]);
  const earned = await pool.query("SELECT COALESCE(SUM(amount),0) as total FROM bonus_earnings WHERE user_id=$1 AND type='parent_referral'", [req.session.parent.phone]);
  res.send(renderPage('Referrals', '<div class="card"><h1>💰 Parent Referrals</h1><div class="stat-num">UGX ' + (earned.rows[0].total || 0) + '</div><p>Your Code: <strong>' + esc(req.session.parent.phone) + '</strong></p></div><div class="card"><h3>Referred (' + refs.rows.length + ')</h3><table><tbody>' + (refs.rows.map(r => '<tr><td>' + esc(r.name || r.phone) + '</td><td>' + r.children + '</td><td class="badge badge-green">+200</td></tr>').join('') || '<tr><td colspan="3">None yet</td></tr>') + '</tbody></table></div>'));
}));
...
app.post('/parent/pay', checkDb, asyncHandler(async (req, res) => {
  ...
  await addDevCommission(Math.round(req.body.amount * DEV_COMMISSION.fee_payment), 'fee_payment', 'Fee commission', ref);
...
app.get('/bonus', requireAuth, checkDb, asyncHandler(async (req, res) => {
  ...
  res.send(renderPage('Rewards', '<div class="hero" style="padding:40px 20px"><h2>My Wallet</h2><div class="stat-num" style="font-size:48px;color:white;-webkit-text-fill-color:white">UGX ' + w.balance + '</div><div style="display:flex;gap:12px;justify-content:center;margin-top:20px"><a href="/bonus/withdraw" class="btn btn-green">Withdraw</a><a href="/bonus/affiliate" class="btn btn-purple">Affiliate</a></div></div><div class="grid"><div class="stat-card" onclick="location.href=\'/videos\'" style="cursor:pointer"><div style="font-size:36px;margin-bottom:8px">🎬</div><div>Videos</div><div class="badge badge-green">+50</div></div><div class="stat-card" onclick="location.href=\'/news\'" style="cursor:pointer"><div style="font-size:36px;margin-bottom:8px">📰</div><div>News</div><div class="badge badge-green">+20</div></div><div class="stat-card" onclick="location.href=\'/downloads\'" style="cursor:pointer"><div style="font-size:36px;margin-bottom:8px">📥</div><div>Downloads</div><div class="badge badge-green">+100</div></div><div class="stat-card" onclick="location.href=\'/games\'" style="cursor:pointer"><div style="font-size:36px;margin-bottom:8px">🎮</div><div>Games</div><div class="badge badge-green">+30</div></div></div><div class="card"><h3>Recent</h3><table><thead><tr><th>Date</th><th>Type</th><th>Amount</th></tr></thead><tbody>' + (rows || '<tr><td colspan="3">None yet</td></tr>') + '</tbody></table></div>', { tenant_name: req.tenant.name }, false, req.lang));
}));
...
app.post('/bonus/withdraw', requireAuth, checkDb, asyncHandler(async (req, res) => {
  const w = (await pool.query('SELECT balance FROM wallets WHERE user_email=$1', [req.session.user.email])).rows[0];
  if (!w || w.balance < req.body.amount || req.body.amount < 5000) return res.send(renderPage('Error', '<div class="card"><h1>Invalid Amount</h1></div>', { tenant_name: req.tenant.name }, false, req.lang));
...
app.get('/videos', (req, res) => {
  ...
  res.send(renderPage('Videos', '<div class="hero" style="padding:30px"><h1>Watch and Earn</h1></div><div class="grid"><div class="card" style="padding:0;overflow:hidden"><iframe width="100%" height="200" src="https://www.youtube.com/embed/dQw4w9WgXcQ" frameborder="0" allowfullscreen></iframe><div style="padding:16px"><h4>Math Basics</h4><a href="/bonus/claim/video/dQw4w9WgXcQ" class="btn btn-green">Claim +50</a></div></div></div>', null, true, l));
});

app.get('/bonus/claim/video/:id', requireAuth, checkDb, asyncHandler(async (req, res) => {
...
app.get('/news', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const c = f.items.slice(0, 10).map(i => '<div class="card"><h4>' + esc(i.title) + '</h4><a href="/bonus/claim/news?url=' + encodeURIComponent(i.link) + '" class="btn btn-orange" target="_blank">Read +20</a></div>').join('');
    res.send(renderPage('News', '<div class="hero" style="padding:30px"><h1>News</h1></div><div class="grid">' + c + '</div>', null, true, l));
  } catch (e) { res.send(renderPage('News', '<div class="card"><h1>Unavailable</h1></div>', null, true, l)); }
}));

app.get('/bonus/claim/news', requireAuth, checkDb, asyncHandler(async (req, res) => {
...
app.get('/games', (req, res) => {
  const l = req.query.lang || detectLang(req);
  res.send(renderPage('Games', '<div class="hero" style="padding:30px"><h1>Games</h1></div><div class="grid"><div class="card" style="text-align:center"><div style="font-size:64px;margin-bottom:12px">🧮</div><h3>Math Quiz</h3><div class="badge badge-gold">+30 UGX</div>' + (req.session.user ? '<a href="/games/play/quiz" class="btn btn-green" style="margin-top:12px">Play</a>' : '') + '</div></div>', null, true, l));
});

app.get('/games/play/:id', requireAuth, checkDb, (req, res) => {
  res.send(renderPage('Playing', '<div class="card" style="max-width:600px;margin:40px auto"><h1>Math Quiz</h1><div id="quiz-area" style="text-align:center;margin:20px 0"><div style="font-size:36px" id="question"></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:20px"><button class="btn" onclick="checkAnswer(this)" id="ans1"></button><button class="btn" onclick="checkAnswer(this)" id="ans2"></button><button class="btn" onclick="checkAnswer(this)" id="ans3"></button><button class="btn" onclick="checkAnswer(this)" id="ans4"></button></div></div><div id="result" style="display:none;text-align:center"><h2>Won!</h2><p class="badge badge-green" style="font-size:18px">+30 UGX</p><a href="/games" class="btn" style="margin-top:20px">Back</a></div></div><script>let q=[],idx=0;for(let i=0;i<5;i++){let a=Math.floor(Math.random()*20)+1,b=Math.floor(Math.random()*20)+1,o=[\'+\',\'-\',\'×\'][Math.floor(Math.random()*3)],ans=o===\'+\'?a+b:o===\'-\'?a-b:a*b;q.push({q:a+\' \'+o+\' \'+b+\' = ?\',ans});}function showQ(){if(idx>=5){document.getElementById(\'quiz-area\').style.display=\'none\';document.getElementById(\'result\').style.display=\'block\';fetch(\'/bonus/claim/game/quiz\').catch(()=>{});return;}let c=q[idx];document.getElementById(\'question\').textContent=c.q;let opts=[c.ans];while(opts.length<4){let w=c.ans+Math.floor(Math.random()*20)-10;if(!opts.includes(w))opts.push(w);}opts.sort(()=>Math.random()-0.5);for(let i=1;i<=4;i++){document.getElementById(\'ans\'+i).textContent=opts[i-1];document.getElementById(\'ans\'+i).dataset.ans=opts[i-1];}}function checkAnswer(btn){idx++;showQ();}showQ();</script>', { tenant_name: req.tenant.name }, false, req.lang));
});

app.get('/bonus/claim/game/:id', requireAuth, checkDb, asyncHandler(async (req, res) => {
...
app.get('/learning', (req, res) => {
  const l = req.query.lang || detectLang(req);
  res.send(renderPage('Learning', '<div class="hero" style="padding:30px"><h1>Learn Anything</h1></div><div class="grid"><div class="card" style="text-align:center"><div style="font-size:48px;margin-bottom:12px">🔢</div><h3>Mathematics</h3></div><div class="card" style="text-align:center"><div style="font-size:48px;margin-bottom:12px">🔬</div><h3>Science</h3></div><div class="card" style="text-align:center"><div style="font-size:48px;margin-bottom:12px">📖</div><h3>English</h3></div></div>', null, true, l));
});
...
app.post('/premium/subscribe', requireAuth, checkDb, asyncHandler(async (req, res) => {
...
app.get('/store/buy/:id', (req, res) => {
  res.send(renderPage('Checkout', '<div class="card" style="max-width:500px;margin:40px auto"><h1>Buy Uniform</h1><p class="stat-num" style="font-size:24px">UGX 45,000</p><form method="POST" action="/store/buy/1"><input name="phone" placeholder="MoMo" required><input name="name" placeholder="Name" required><button class="btn btn-green" style="width:100%">Pay</button></form></div>', null, true));
});
...
app.use('/app', requireAuth, asyncHandler(async (req, res, next) => {
  try {
    if (req.session.user && req.tenant && req.tenant.plan === 'free' && req.tenantId) {
      ...
      if (students > 100) {
        ...
        await sendPushToUser(req.session.user.email, 'Upgrade Available', 'You have 100+ students. Upgrade to Basic for UGX 50,000/mo.');
      }
    }
  } catch (e) { console.error('Auto-scale err', e.message); }
  next();
}));
...
app.post('/app/students/add', requireAuth, requireStaff, checkDb, asyncHandler(async (req, res) => {
  await pool.query('INSERT INTO students (tenant_id, name, class, guardian_name, guardian_phone) VALUES ($1,$2,$3,$4,$5) RETURNING *', [req.tenantId, req.body.name, req.body.class, req.body.guardian_name, req.body.guardian_phone]);
  if (req.body.guardian_phone && isValidPhone(req.body.guardian_phone)) await sendSMS(req.body.guardian_phone, req.body.name + ' registered. Track fees: https://' + req.headers.host + '/parent/login');
  if (req.session.user.role === 'teacher') {
    try {
      ...
      if (cnt >= 10 && !paid) {
        await addBonus(req.session.user.email, req.tenantId, 5000, 'teacher_milestone', 'Added 10+ students');
        await sendPushToUser(req.session.user.email, '🎉 Bonus!', 'You earned UGX 5,000!');
      }
    } catch (e) { console.error('Milestone check err:', e.message); }
  }
...
app.post('/app/students/bulk', requireAuth, requireAdmin, upload.single('csv'), checkDb, asyncHandler(async (req, res) => {
  ...
  for (const row of results) {
    if (row.name) { await pool.query('INSERT INTO students (tenant_id, name, class, guardian_name, guardian_phone) VALUES ($1,$2,$3,$4,$5)', [req.tenantId, row.name, row.class, row.guardian_name, row.guardian_phone]); added++; }
  }
  res.send(renderPage('Uploaded', '<div class="card" style="text-align:center"><h1>Imported ' + added + ' Students!</h1><a href="/app/students" class="btn">View Students</a></div>', { tenant_name: req.tenant.name }, false, req.lang));
}));
...
app.get('/learning/live', requireAuth, checkDb, asyncHandler(async (req, res) => {
  ...
  res.send(renderPage('Live Classes', '<div class="hero" style="padding:30px"><h1>Live Classes</h1>' + (['teacher', 'admin', 'super_admin'].includes(req.session.user.role) ? '<a href="/learning/live/create" class="btn btn-green" style="margin-top:12px">Create Class</a>' : '') + '</div><div class="grid">' + (classes.rows.map(c => '<div class="card"><div class="badge badge-blue" style="margin-bottom:8px">' + new Date(c.scheduled_at).toLocaleDateString() + '</div><h3>' + esc(c.title) + '</h3><p style="color:#64748b">' + esc(c.subject) + ' - ' + esc(c.class) + '</p><p style="color:#64748b">👨‍🏫 ' + esc(c.teacher_name) + ' | 👥 ' + c.students + '</p><div class="stat-num" style="font-size:20px;margin:12px 0">UGX ' + c.price + '</div><a href="/learning/live/join/' + c.id + '" class="btn btn-green" style="width:100%">Join</a></div>').join('') || '<div class="card"><p>No live classes scheduled</p></div>') + '</div>', { tenant_name: req.tenant.name }, false, req.lang));
}));

app.get('/learning/live/create', requireAuth, requireStaff, (req, res) => {
  res.send(renderPage('Create Class', '<div class="card" style="max-width:600px"><h1>Create Live Class</h1><p style="color:#64748b">You earn 80%. Platform takes 20%.</p><form method="POST" action="/learning/live/create"><input name="title" placeholder="Title" required><input name="subject" placeholder="Subject" required><input name="class" placeholder="Class (e.g. P.4)" required><input name="price" type="number" value="1000" min="500" required><input name="scheduled_at" type="datetime-local" required><button class="btn btn-green" style="width:100%">Create</button></form></div>', { tenant_name: req.tenant.name }, false, req.lang));
});
...
app.post('/learning/live/create', requireAuth, requireStaff, checkDb, asyncHandler(async (req, res) => {
  ...
  await sendPushToUser(req.session.user.email, 'Class Created', req.body.title);
...
app.get('/learning/live/join/:id', requireAuth, checkDb, asyncHandler(async (req, res) => {
  ...
  if (!paid.rows[0] && !isTeacher && cls.price > 0) return res.send(renderPage('Payment Required', '<div class="card" style="max-width:500px;margin:40px auto;text-align:center"><h1>Join ' + esc(cls.title) + '</h1><div class="stat-num" style="margin:20px 0">UGX ' + cls.price + '</div><form method="POST" action="/learning/live/pay/' + cls.id + '"><input name="phone" placeholder="MoMo" required><button class="btn btn-green" style="width:100%">Pay & Join</button></form></div>', { tenant_name: req.tenant.name }, false, req.lang));
  res.send(renderPage(cls.title, '<div class="card" style="padding:0;overflow:hidden"><div style="background:#1e293b;color:white;padding:16px"><h2>' + esc(cls.title) + '</h2><p>' + esc(cls.subject) + ' - ' + esc(cls.class) + '</p></div><iframe src="https://meet.jit.si/' + cls.jitsi_room + '" style="width:100%;height:600px;border:0" allow="camera;microphone;fullscreen"></iframe></div>', { tenant_name: req.tenant.name }, false, req.lang));
}));
...
app.post('/learning/live/pay/:id', requireAuth, checkDb, asyncHandler(async (req, res) => {
  ...
  await pool.query('INSERT INTO class_payments (class_id, student_email, amount, phone, reference, status) VALUES ($1,$2,$3,$4,$5,$6)', [cls.id, req.session.user.email, cls.price, req.body.phone, ref, 'pending']);
  await addDevCommission(commission, 'live_class', 'Live class: ' + cls.title, ref);
  await addBonus(cls.teacher_email, cls.tenantId, cls.price - commission, 'teaching', 'Taught: ' + cls.title);
  if (MOMO_CONFIG.apiKey === 'demo') {
...
app.get('/app/analytics', requireAuth, requireAdmin, checkDb, asyncHandler(async (req, res) => {
  ...
  const topStudents = await pool.query("SELECT s.name, s.class, AVG(g.score) as avg_score FROM students s JOIN grades g ON s.id=g.student_id WHERE s.tenant_id=$1 GROUP BY s.id, s.name, s.class HAVING COUNT(g.id) > 0 ORDER BY avg_score DESC LIMIT 10", [req.tenantId]);
  const attRows = attData.rows.map(r => { const pct = r.total > 0 ? Math.round((r.present / r.total) * 100) : 0; return '<tr><td>' + esc(r.class || 'N/A') + '</td><td>' + r.present + '/' + r.total + '</td><td style="width:40%"><div style="background:#e2e8f0;border-radius:20px;height:24px"><div style="background:' + (pct > 75 ? '#16a34a' : pct > 50 ? '#f59e0b' : '#dc2626') + ';width:' + pct + '%;height:100%;border-radius:20px"></div></div></td></tr>'; }).join('');
  res.send(renderPage('Analytics', '<div class="hero" style="padding:30px"><h1>Analytics</h1></div><div class="grid"><div class="card"><h3>Fee Collection (6mo)</h3><canvas id="feeChart"></canvas></div><div class="card"><h3>Attendance</h3><table><thead><tr><th>Class</th><th>Rate</th></tr></thead><tbody>' + attRows + '</tbody></table></div></div><div class="card"><h3>Top Performers</h3><table><thead><tr><th>#</th><th>Student</th><th>Class</th><th>Avg</th></tr></thead><tbody>' + topStudents.rows.map((r, i) => '<tr><td>' + (i + 1) + '</td><td>' + esc(r.name) + '</td><td>' + esc(r.class) + '</td><td class="badge badge-gold">' + Math.round(r.avg_score) + '</td></tr>').join('') + '</tbody></table></div><script src="https://cdn.jsdelivr.net/npm/chart.js"></script><script>new Chart(document.getElementById("feeChart"),{type:"line",data:{labels:' + JSON.stringify(feeData.rows.map(r => new Date(r.month).toLocaleDateString('en-US', { month: 'short' }))) + ',datasets:[{label:"UGX",data:' + JSON.stringify(feeData.rows.map(r => r.total)) + ',borderColor:"#3b82f6",tension:0.4}]})</script>', { tenant_name: req.tenant.name }, false, req.lang));
}));
...
app.get('/api/cron/daily', asyncHandler(async (req, res) => {
  ...
  if (wallet.balance >= AUTO_PAYOUT_THRESHOLD) {
    payout = Math.floor(wallet.balance * AUTO_PAYOUT_PERCENTAGE);
    await pool.query('UPDATE platform_wallet SET balance = balance - $1 WHERE id=1', [payout]);
    await pool.query('INSERT INTO withdrawals (user_email, amount, fee, net_amount, phone, status) VALUES ($1,$2,$3,$4,$5,$6)', ['DEVELOPER_PAYOUT', payout, 0, payout, wallet.developer_momo || '0789736737', 'auto_pending']);
  }
  await pool.query("DELETE FROM session WHERE expire < NOW() - INTERVAL '7 days'");
  res.json({ ok: true, payout, balance: wallet.balance });
}));

app.get('/demo', (req, res) => {
  res.send(renderPage('Book Demo', '<div class="hero" style="padding:40px 20px"><h1>📞 Free Demo for Your School</h1><p>5 min setup.</p></div><div class="card" style="max-width:600px;margin:0 auto"><form method="POST" action="/demo"><input name="school_name" placeholder="School Name" required><input name="headteacher_name" placeholder="Headteacher" required><input name="phone" placeholder="WhatsApp 07XX" required><input name="students" type="number" placeholder="Students" required><button class="btn btn-green" style="width:100%;font-size:18px">Book Demo</button></form></div>', null, true));
});

app.post('/demo', checkDb, asyncHandler(async (req, res) => {
  const subdomain = req.body.school_name.toLowerCase().replace(/[^a-z0-9]/g, '').substring(0, 15);
  const signup_code = subdomain.toUpperCase().substring(0, 8) + Math.floor(Math.random() * 100);
  try {
    const t = await pool.query('INSERT INTO tenants (name, subdomain, plan, momo_number, signup_code) VALUES ($1,$2,$3,$4,$5) RETURNING id', [req.body.school_name.trim(), subdomain, 'free', req.body.phone, signup_code]);
    await pool.query('INSERT INTO settings (tenant_id, signup_code) VALUES ($1,$2)', [t.rows[0].id, signup_code]);
    await sendWhatsApp(req.body.phone, '✅ ' + req.body.school_name + ' is LIVE!\nTeacher Code: *' + signup_code + '*\nLink: https://' + req.headers.host + '/school/' + subdomain);
    await sendWhatsApp('0789736737', '🔥 NEW: ' + req.body.school_name + '\nStudents: ' + req.body.students + '\nCode: ' + signup_code);
    res.send(renderPage('Success', '<div class="card" style="text-align:center;max-width:600px;margin:40px auto"><div style="font-size:60px">🎉</div><h1>' + esc(req.body.school_name) + ' is Live!</h1><p>Teacher Code: <strong class="badge badge-gold" style="font-size:24px">' + signup_code + '</strong></p><a href="/demo" class="btn btn-green" style="margin-top:20px">Add Another</a></div>', null, true));
  } catch (e) {
    if (e.code === '23505') return res.send(renderPage('Error', '<div class="card"><h1>Name taken</h1></div>', null, true));
    throw e;
  }
}));
...
app.get('/super-admin', requireAuth, requireRole('super_admin'), checkDb, asyncHandler(async (req, res) => {
  ...
  const revTotal = (await pool.query('SELECT COALESCE(SUM(amount),0) as t FROM developer_revenue')).rows[0].t;
  const pend = (await pool.query("SELECT COUNT(*) as c, COALESCE(SUM(amount),0) as t FROM withdrawals WHERE status='pending'")).rows[0];
  const schools = (await pool.query("SELECT COUNT(*) as c FROM tenants")).rows[0].c;
  ...
  res.send(renderPage('Super Admin', '<div class="hero" style="background:linear-gradient(135deg,#dc2626,#ef4444);padding:30px"><h1>👑 Platform Control</h1></div><div class="stats"><div class="stat-card"><div class="stat-num">UGX ' + rev.toLocaleString() + '</div><div>30 Days</div></div><div class="stat-card"><div class="stat-num">UGX ' + revTotal.toLocaleString() + '</div><div>Total</div></div><div class="stat-card"><div class="stat-num">UGX ' + wallet.toLocaleString() + '</div><div>Wallet</div></div><div class="stat-card"><div class="stat-num">' + pend.c + '</div><div>Pending (' + Number(pend.t).toLocaleString() + ')</div></div><div class="stat-card"><div class="stat-num">' + schools + '</div><div>Schools</div></div><div class="stat-card"><div class="stat-num">' + users + '</div><div>Users</div></div></div><div class="grid"><div class="card"><h3>Actions</h3><div style="display:flex;flex-direction:column;gap:8px;margin-top:12px"><a href="/demo" class="btn btn-green">Create School</a><a href="/super-admin/broadcast" class="btn btn-purple">Broadcast</a><a href="/super-admin/payout-developer" class="btn btn-red">Payout</a></div></div><div class="card"><h3>Auto-Payout</h3><p>When wallet ≥ UGX 50,000: Auto-pays 95% daily.</p><p>Cron: GET /api/cron/daily?secret=YOUR_SECRET</p></div></div>', { tenant_name: req.tenant.name }));
}));

app.get('/super-admin/payout-developer', requireAuth, requireRole('super_admin'), checkDb, asyncHandler(async (req, res) => {
  const wallet = (await pool.query('SELECT balance FROM platform_wallet WHERE id=1')).rows[0].balance;
  if (wallet < 1000) return res.redirect('/super-admin');
  await pool.query('UPDATE platform_wallet SET balance = 0 WHERE id=1');
  await pool.query('INSERT INTO withdrawals (user_email, amount, fee, net_amount, phone, status) VALUES ($1,$2,$3,$4,$5,$6)', ['DEVELOPER_MANUAL', wallet, 0, wallet, '0789736737', 'paid']);
...
app.post('/super-admin/broadcast', requireAuth, requireRole('super_admin'), checkDb, asyncHandler(async (req, res) => {
  ...
  if (req.body.target === 'teachers') query += ' AND role=\'teacher\'';
  if (req.body.target === 'admins') query += ' AND role IN (\'admin\',\'super_admin\')';
  const { rows } = await pool.query(query);
  let sent = 0;
  for (const r of rows) { await sendWhatsApp(r.phone, req.body.message); await new Promise(res => setTimeout(res, 1000)); sent++; }
  res.send(renderPage('Sent', '<div class="card"><h1>✅ Sent to ' + sent + ' numbers</h1><a href="/super-admin" class="btn">Back</a></div>', { tenant_name: req.tenant.name }));
}));
...
app.get('/school/:sub', checkDb, asyncHandler(async (req, res) => {
  ...
  res.send(renderPage(t.name, '<div class="hero" style="background:linear-gradient(135deg,#1e40af,#3b82f6);padding:60px 20px"><h1>' + esc(t.name) + '</h1><p style="font-size:20px">' + esc(t.school_motto || 'Excellence') + '</p><div style="display:flex;gap:12px;justify-content:center;margin-top:20px"><a href="/parent/login" class="btn btn-green">Parent Portal</a><a href="/login" class="btn">Staff Login</a></div></div><div class="stats"><div class="stat-card"><div class="stat-num">' + stats.rows[0].students + '</div><div>Students</div></div></div><div class="card"><h2>About</h2><p>' + esc(t.about_text || 'Welcome.') + '</p></div><div class="card" style="text-align:center"><h3>Join Us</h3><p>Teacher Code: <strong class="badge badge-gold">' + esc(t.signup_code || 'Contact admin') + '</strong></p><a href="/signup" class="btn btn-green">Apply</a></div>', null, true));
}));
...
app.get('/sitemap.xml', checkDb, asyncHandler(async (req, res) => {
  ...
  res.header('Content-Type', 'application/xml').send('<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>https://' + req.headers.host + '/</loc></url>' + schools.rows.map(s => '<url><loc>https://' + req.headers.host + '/school/' + s.subdomain + '</loc></url>').join('') + '</urlset>');
}));
...
app.post('/create-site', checkDb, asyncHandler(async (req, res) => {
  try {
    ...
    if (e.code === '23505') return res.send(renderPage('Error', '<div class="card"><h1>Taken</h1></div>', null, true));
    throw e;
  }
}));
...
app.post('/api/momo/webhook', asyncHandler(async (req, res) => {
  const { reference, status } = req.body || {};
  if (!reference || !status) return res.status(400).json({ error: 'Missing data' });
