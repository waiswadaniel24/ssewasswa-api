/**
 * SSEWASSWA Network v6.0 (FINAL COMPLETE)
...
const DEVELOPER_EMAIL='ssewasswa@gmail.com',DEVELOPER_PHONE='0789736737',DEVELOPER_RATE=0.05,PLATFORM_COMMISSION=0.10;
...
const SUBSCRIPTION_PLANS={
  ...
  school_pro:{name:'School Pro',price:50000,currency:'UGX',features:['Unlimited students','All portals','SMS alerts','USSD','Offline'],portals:['academics','stores','admin','papers','funds','reports','finance','marketplace','programs','news','ads','entertainment']},
  org_basic:{name:'Org Basic',price:30000,currency:'UGX',features:['50 members','Project tracking'],portals:['dashboard','members','finance','projects','public']},
  org_pro:{name:'Org Pro',price:100000,currency:'UGX',features:['Unlimited members','Donor portal','Grant applications'],portals:['dashboard','members','finance','reports','projects','marketplace','programs','news','ads']},
  business:{name:'Business',price:20000,currency:'UGX',features:['Unlimited products','0% commission first month'],portals:['seller','orders','products','wallet','ads','analytics']},
  donor:{name:'Donor Premium',price:50000,currency:'UGX',features:['Post grants','Review applications','Impact tracking'],portals:['dashboard','opportunities','history','impact']}
};
...
function renderPage(title,content,user){
  ...
  if(user){
    ...
    nav=`<div style="background:linear-gradient(135deg,#1e40af,#3b82f6);color:white;padding:12px 20px;margin:0 0 24px 0"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px"><div style="display:flex;align-items:center;gap:10px"><b>${esc(user.tenant_name)}</b> - ${esc(user.name)} ${user.verified?'✅':''}</div><div style="display:flex;gap:12px;flex-wrap:wrap;font-size:13px">${Object.entries(portals).map(([k,v])=>`<a href="/portal/${k}" style="color:white;text-decoration:none">${v}</a>`).join('')}${user.role==='super_admin'?`<a href="/dev/master" style="color:#fef3c7;text-decoration:none;font-weight:bold">🔴 Dev Portal</a>`:''}<a href="/logout" style="color:white;text-decoration:none">Logout</a></div></div></div>`;
  }
...
app.get('/',ah(async(req,res)=>{
  const stats={
    schools:(await pool.query("SELECT COUNT(*) as c FROM tenants WHERE type='school' AND status='active'")).rows[0].c,
...
app.get('/marketplace',ah(async(req,res)=>{
  ...
  res.send(renderPage('Marketplace',`<div class="hero"><h1>🛒 Marketplace</h1><p>Buy from schools & businesses</p></div><div class="grid">${products.map(p=>`<div class="card"><img src="${p.image_url||'/img/default.jpg'}" style="width:100%;height:200px;object-fit:cover;border-radius:12px"><h3>${esc(p.name)}</h3><p style="color:#64748b">by ${esc(p.seller_name)}</p><p style="font-size:24px;color:#16a34a;font-weight:bold">UGX ${p.price.toLocaleString()}</p><a href="/product/${p.id}" class="btn btn-green">Buy Now</a></div>`).join('')}</div>`,null,true));
}));

app.get('/product/:id',ah(async(req,res)=>{
  const p=(await pool.query('SELECT p.*,t.name as seller_name,t.subdomain FROM products p JOIN tenants t ON p.tenant_id=t.id WHERE p.id=$1',[req.params.id])).rows[0];
...
app.get('/blog',ah(async(req,res)=>{
  ...
  res.send(renderPage('Blog',`<div class="hero"><h1>News & Stories</h1></div><div class="grid">${posts.map(p=>`<div class="card"><img src="${p.image_url||'/img/default.jpg'}" style="width:100%;height:200px;object-fit:cover;border-radius:12px"><h3>${esc(p.title)}</h3><p style="color:#64748b;font-size:13px">By ${esc(p.author_name)} • ${new Date(p.created_at).toDateString()}</p><p>${esc(p.summary)}</p><a href="/blog/${p.id}" class="btn">Read More</a></div>`).join('')}</div>`,null,true));
}));

app.get('/blog/:id',ah(async(req,res)=>{
  const post=(await pool.query('SELECT n.*,t.name as author_name FROM news_articles n JOIN tenants t ON n.tenant_id=t.id WHERE n.id=$1',[req.params.id])).rows[0];
  await pool.query('UPDATE news_articles SET views=views+1 WHERE id=$1',[req.params.id]);
  res.send(renderPage(post.title,`<div class="card"><img src="${post.image_url}" style="width:100%;border-radius:16px"><h1>${esc(post.title)}</h1><p style="color:#64748b">By ${esc(post.author_name)} • ${new Date(post.created_at).toDateString()} • ${post.views} views</p><div style="margin-top:20px">${post.content}</div></div>`,null,true));
}));
...
app.get('/schools',ah(async(req,res)=>{
  ...
  res.send(renderPage('Schools',`<div class="hero"><h1>🏫 Find Schools</h1></div><div class="grid">${schools.map(s=>`<div class="card"><img src="${s.logo_url||'/img/school.jpg'}" style="width:100%;height:150px;object-fit:cover;border-radius:12px"><h3>${esc(s.name)} ${s.verified?'✅':''}</h3><p>${esc(s.description||'').substring(0,100)}</p><a href="/s/${s.subdomain}" class="btn">Visit Site</a></div>`).join('')}</div>`,null,true));
}));

app.get('/s/:subdomain',ah(async(req,res)=>{
  const t=(await pool.query('SELECT * FROM tenants WHERE subdomain=$1 AND type=$2',[req.params.subdomain,'school'])).rows[0];
  if(!t)return res.status(404).send('School not found');
  const social=JSON.parse(t.social_links||'{}');
  res.send(`<!doctype html><html><head><title>${esc(t.name)}</title><meta name="viewport" content="width=device-width,initial-scale=1"><style>${renderPage('','',null).match(/<style>(.*?)<\/style>/s)[1]}</style></head><body><div class="container"><header style="text-align:center;padding:40px 0"><h1>${esc(t.name)} ${t.verified?'✅':''}</h1><p>${esc(t.tagline||'Excellence in Education')}</p><div style="margin:20px 0">${t.show_donate?`<a href="/pay/${t.id}" class="btn btn-green">Donate</a>`:''}<a href="/s/${t.subdomain}/enroll" class="btn">Enroll</a></div></header><div class="grid"><div class="card"><h3>About Us</h3><p>${esc(t.about_us||t.description||'')}</p></div><div class="card"><h3>Contact</h3><p>📞 ${esc(t.contact_phone||'')}</p><p>📧 ${esc(t.contact_email||'')}</p><p>📍 ${esc(t.address||'')}</p></div></div>${t.show_news?`<div class="card"><h3>Latest News</h3><div id="news"></div></div>`:''}</div><script>fetch('/api/news/${t.id}').then(r=>r.json()).then(d=>document.getElementById('news').innerHTML=d.map(n=>'<div style="padding:10px;border-bottom:1px solid #e2e8f0"><h4>'+n.title+'</h4><p>'+n.summary+'</p></div>').join(''))</script></body></html>`);
}));
...
app.get('/s/:subdomain/enroll',ah(async(req,res)=>{
  ...
  res.send(renderPage('Enroll',`<div class="card" style="max-width:600px;margin:40px auto"><h1>Enroll at ${esc(t.name)}</h1><form method="POST" action="/s/${t.subdomain}/enroll" enctype="multipart/form-data"><h3>Student Details</h3><input name="student_name" placeholder="Full Name" required><input name="dob" type="date" required><select name="gender" required><option value="">Gender</option><option>Male</option><option>Female</option></select><select name="class_applying" required><option value="">Class</option><option>Baby</option><option>Middle</option><option>Top</option><option>P.1</option><option>P.2</option><option>P.3</option><option>P.4</option><option>P.5</option><option>P.6</option><option>P.7</option></select><h3>Parent/Guardian</h3><input name="parent_name" placeholder="Parent Name" required><input name="parent_phone" placeholder="Phone: 078..." required><input name="parent_email" type="email" placeholder="Email"><h3>Documents</h3><label>Birth Certificate</label><input type="file" name="birth_cert"><label>Previous Report</label><input type="file" name="report_card"><label>Photo</label><input type="file" name="photo"><button class="btn btn-green">Submit Application</button></form></div>`,null,true));
}));

app.post('/s/:subdomain/enroll',upload.fields([{name:'birth_cert',maxCount:1},{name:'report_card',maxCount:1},{name:'photo',maxCount:1}]),ah(async(req,res)=>{
  const t=(await pool.query('SELECT id FROM tenants WHERE subdomain=$1',[req.params.subdomain])).rows[0];
  ...
  res.send(`<script>alert('Application submitted! We will contact you.');window.location='/s/${req.params.subdomain}'</script>`);
}));

app.get('/donors/public',ah(async(req,res)=>{
  const opps=(await pool.query("SELECT f.*,t.name as org_name FROM fund_opportunities f JOIN tenants t ON f.tenant_id=t.id WHERE f.active=true ORDER BY f.deadline ASC")).rows;
  res.send(renderPage('Funding Opportunities',`<div class="hero"><h1>💰 Fund Education</h1></div><div class="grid">${opps.map(o=>`<div class="card"><h3>${esc(o.title)}</h3><p style="color:#64748b">by ${esc(o.org_name)}</p><p>${esc(o.summary)}</p><p style="font-size:24px;color:#16a34a;font-weight:bold">USD ${o.amount.toLocaleString()}</p><p>Deadline: ${new Date(o.deadline).toDateString()}</p><a href="/register?type=donor" class="btn btn-green">Apply to Fund</a></div>`).join('')}</div>`,null,true));
}));
...
app.get('/pay/:tenant_id',ah(async(req,res)=>{
  ...
  res.send(renderPage('Pay',`<div class="card" style="max-width:500px;margin:40px auto"><h2>Pay ${esc(t.name)}</h2><div style="text-align:center;margin:15px 0"><span class="badge badge-green">🔒 SSL Encrypted</span><span class="badge badge-blue">🛡️ PCI Compliant</span></div><form method="POST" action="/pay/${t.id}"><label>Amount (UGX)</label><input name="amount" type="number" min="1000" required><label>Purpose</label><select name="purpose" required><option value="fees">School Fees</option><option value="donation">Donation</option><option value="other">Other</option></select><label>Your Name</label><input name="name" required><label>Phone</label><input name="phone" placeholder="078..." required><label>Email (for receipt)</label><input name="email" type="email" required><label>Payment Method</label><select name="gateway" required><option value="mtn">MTN MoMo</option><option value="airtel">Airtel Money</option></select><button class="btn btn-green" style="width:100%">Pay Securely</button></form></div>`,null,true));
}));
...
app.post('/pay/:tenant_id',ah(async(req,res)=>{
  ...
  res.send(`<script>alert('Payment initiated. Check ${phone} to approve. Receipt sent to ${email}');window.location='/pay/success/${ref}'</script>`);
}));
...
app.post('/login',ah(async(req,res)=>{
  ...
  if(u.two_fa_enabled&&!otp){await sendSMS(u.phone,`SSEWASSWA Code: ${Math.floor(100000+Math.random()*900000)}`);return res.json({require_otp:true});}
  req.session.user=u;
...
app.post('/dev/scrape',requireAuth,requireDeveloper,ah(async(req,res)=>{
  ...
  let count=0;
  if(type==='news'){
    $('article,.news-item').each(async(i,el)=>{
      const title=$(el).find('h1,h2,h3').first().text().trim();
      const summary=$(el).find('p').first().text().trim().substring(0,200);
      if(title){await pool.query('INSERT INTO news_articles(tenant_id,title,summary,published)VALUES($1,$2,$3,true)',[1,title,summary]);count++;}
    });
  }
  res.json({ok:true,scraped:count});
}));
app.get('/portal/admin',requireAuth,requireRole('school_admin','org_admin','super_admin'),ah(async(req,res)=>{
  const stats={
    students:(await pool.query('SELECT COUNT(*) as c FROM students WHERE tenant_id=$1',[req.session.user.tenant_id])).rows[0].c,
    balance:(await pool.query('SELECT wallet_balance FROM tenants WHERE id=$1',[req.session.user.tenant_id])).rows[0].wallet_balance,
    pending_fees:(await pool.query('SELECT COALESCE(SUM(amount),0) as t FROM fees WHERE tenant_id=$1 AND paid=false',[req.session.user.tenant_id])).rows[0].t,
    revenue_30d:(await pool.query(`SELECT COALESCE(SUM(amount),0) as t FROM developer_revenue WHERE tenant_id=$1 AND created_at>NOW()-INTERVAL '30 days'`,[req.session.user.tenant_id])).rows[0].t
  };
  res.send(renderPage('Admin Dashboard',`<div class="stats"><div class="stat-card"><div class="stat-num">${stats.students}</div><div>Students</div></div><div class="stat-card"><div class="stat-num">UGX ${stats.balance.toLocaleString()}</div><div>Wallet</div></div><div class="stat-card"><div class="stat-num">UGX ${stats.pending_fees.toLocaleString()}</div><div>Pending Fees</div></div><div class="stat-card"><div class="stat-num">UGX ${Math.round(stats.revenue_30d).toLocaleString()}</div><div>30-Day Revenue</div></div></div><div class="grid"><a href="/portal/academics" class="card"><h3>📚 Academics</h3><p>Marks, attendance, reports</p></a><a href="/portal/finance" class="card"><h3>💰 Finance</h3><p>Fees, expenses, wallet</p></a><a href="/portal/students" class="card"><h3>👥 Students</h3><p>Add, edit, bulk import</p></a><a href="/portal/marketplace" class="card"><h3>🛒 Marketplace</h3><p>Sell products</p></a></div>`,req.session.user));
}));

app.get('/portal/academics',requireAuth,requirePortal('academics'),ah(async(req,res)=>{
  const classes=(await pool.query('SELECT DISTINCT class FROM students WHERE tenant_id=$1',[req.session.user.tenant_id])).rows;
  res.send(renderPage('Academics',`<div class="card"><h2>Enter Marks</h2><form method="POST" action="/portal/academics/marks"><select name="class" required><option value="">Select Class</option>${classes.map(c=>`<option>${c.class}</option>`).join('')}</select><select name="subject" required><option value="">Subject</option><option>Math</option><option>English</option><option>Science</option></select><select name="term" required><option value="1">Term 1</option><option value="2">Term 2</option><option value="3">Term 3</option></select><div id="students-list"></div><button class="btn btn-green">Save Marks</button></form></div><div class="card"><h3>Bulk Import CSV</h3><form method="POST" action="/portal/students/bulk" enctype="multipart/form-data"><input type="file" name="csv" accept=".csv" required><button class="btn">Import Students</button></form></div>`,req.session.user,'academics'));
}));

app.post('/portal/academics/marks',requireAuth,requirePortal('academics'),ah(async(req,res)=>{
  const{marks}=req.body; // Array of {student_id,score}
...
app.get('/cart',ah(async(req,res)=>{
  ...
  const products=cart.length?await Promise.all(cart.map(async(c)=>{const p=(await pool.query('SELECT * FROM products WHERE id=$1',[c.product_id])).rows[0];return{...p,quantity:c.quantity}})):[];
...
app.get('/checkout',ah(async(req,res)=>{
  ...
  const products=await Promise.all(cart.map(async(c)=>{const p=(await pool.query('SELECT * FROM products WHERE id=$1',[c.product_id])).rows[0];return{...p,quantity:c.quantity}}));
  const total=products.reduce((sum,p)=>sum+p.price*p.quantity,0);
...
app.post('/checkout',ah(async(req,res)=>{
  ...
  const apps=(await pool.query('SELECT a.*,f.title as opp_title FROM fund_applications a JOIN fund_opportunities f ON a.opportunity_id=f.id WHERE f.tenant_id=$1 ORDER BY a.created_at DESC',[req.session.user.tenant_id])).rows;
...
app.get('/donate/recurring',requireAuth,ah(async(req,res)=>{
  res.send(renderPage('Recurring Donations',`<div class="card" style="max-width:500px;margin:40px auto"><h2>Set Up Monthly Donation</h2><form method="POST" action="/donate/recurring"><label>Amount (UGX)</label><input name="amount" type="number" min="5000" required><label>Frequency</label><select name="frequency" required><option value="monthly">Monthly</option><option value="quarterly">Quarterly</option></select><label>To Opportunity</label><select name="opportunity_id" required>${(await pool.query('SELECT id,title FROM fund_opportunities WHERE active=true')).rows.map(o=>`<option value="${o.id}">${esc(o.title)}</option>`).join('')}</select><button class="btn btn-green" style="width:100%">Activate Recurring</button></form></div>`,req.session.user));
}));

app.post('/donate/recurring',requireAuth,ah(async(req,res)=>{
  const{opportunity_id,amount,frequency}=req.body;
  const next=new Date();if(frequency==='monthly')next.setMonth(next.getMonth()+1);else next.setMonth(next.getMonth()+3);
...
app.get('/sw.js',(req,res)=>{
  ...
  res.send(`const CACHE='v6.0';self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(['/','/offline','/manifest.json']))));self.addEventListener('fetch',e=>{if(e.request.method==='GET')e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request).catch(()=>caches.match('/offline'))))});self.addEventListener('sync',e=>{if(e.tag==='sync-data')e.waitUntil(syncOfflineData())});async function syncOfflineData(){const db=await indexedDB.open('ssewasswa',1);}`);
});

app.get('/manifest.json',(req,res)=>res.json({name:'SSEWASSWA Network',short_name:'SSEWASSWA',start_url:'/',display:'standalone',background_color:'#1e40af',theme_color:'#1e40af',icons:[{src:'/icon-192.png',sizes:'192x192',type:'image/png'},{src:'/icon-512.png',sizes:'512x512',type:'image/png'}]}));

app.get('/offline',(req,res)=>res.send(`<!doctype html><html><head><title>Offline</title><style>body{font-family:system-ui;text-align:center;padding:50px}</style></head><body><h1>📡 You're Offline</h1><p>Data will sync when connection returns.</p><a href="/">Retry</a></body></html>`));
...
app.post('/api/sync',requireAuth,ah(async(req,res)=>{
  ...
  if(type==='grades'){for(const g of data)await pool.query('INSERT INTO grades(tenant_id,student_id,subject,score,grade,term,teacher_id)VALUES($1,$2,$3,$4,$5,$6,$7)ON CONFLICT DO NOTHING',[g.tenant_id,g.student_id,g.subject,g.score,g.grade,g.term,g.teacher_id]);}
  else if(type==='attendance'){for(const a of data)await pool.query('INSERT INTO attendance(tenant_id,student_id,date,status,marked_by)VALUES($1,$2,$3,$4,$5)ON CONFLICT DO NOTHING',[a.tenant_id,a.student_id,a.date,a.status,a.marked_by]);}
...
cron.schedule('0 9 * * *',async()=>{
  ...
  for(const f of fees){await pool.query('UPDATE tenants SET wallet_balance=wallet_balance+$1 WHERE id=$2',[f.dev_total,f.tenant_id]);}
});
...
async function initDB(){
  ...
  try{
    ...
    await c.query(`CREATE TABLE IF NOT EXISTS tenants (id SERIAL PRIMARY KEY, name TEXT, subdomain TEXT UNIQUE, type TEXT DEFAULT 'school', description TEXT, gateway TEXT DEFAULT 'mtn', momo_number_encrypted TEXT, momo_name TEXT, wallet_balance INTEGER DEFAULT 0, status TEXT DEFAULT 'active', subscription_plan TEXT DEFAULT 'school_free', subscription_status TEXT DEFAULT 'trial', verified BOOLEAN DEFAULT false, features_enabled JSONB DEFAULT '{}', social_links JSONB DEFAULT '{}', about_us TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
    await c.query(`CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, tenant_id INT, email TEXT UNIQUE, password_hash TEXT, role TEXT, portals TEXT[], name TEXT, phone_encrypted TEXT, approved BOOLEAN DEFAULT false, verified BOOLEAN DEFAULT false, wallet_balance INTEGER DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW())`);
    ...
    console.log('DB v6.0 Ready');
  }catch(e){await c.query('ROLLBACK');console.error(e);process.exit(1)}finally{c.release()}
}

initDB().then(()=>app.listen(PORT,()=>console.log(`SSEWASSWA v6.0 LIVE on ${PORT}`)));
