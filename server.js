...
app.get('/learning', async (req, res) => {    const l = req.query.lang || detectLang(req);    const subjects = ['Mathematics|🔢', 'Science|🔬', 'English|📖'];   const subjectCards = subjects.map(c => {     const parts = c.split('|');     const name = parts[0];     const icon = parts[1];     return '<div class="card" style="text-align:center;cursor:pointer"><div style="font-size:48px;margin-bottom:12px">' + icon + '</div><h3>' + name + '</h3></div>';   }).join('');   res.send(renderPage('Learning 📚', '<div class="hero" style="padding:30px"><h1>📚 Learn Anything</h1></div><div class="grid">' + subjectCards + '</div>', null, true, l);  });
