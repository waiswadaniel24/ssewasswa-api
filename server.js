...
app.get('/learning', async (req, res) => {
  const l = req.query.lang || detectLang(req);
  const cards = [];
  cards.push('<div class="card" style="text-align:center;cursor:pointer"><div style="font-size:48px;margin-bottom:12px">🔢</div><h3>Mathematics</h3></div>');
  cards.push('<div class="card" style="text-align:center;cursor:pointer"><div style="font-size:48px;margin-bottom:12px">🔬</div><h3>Science</h3></div>');
  cards.push('<div class="card" style="text-align:center;cursor:pointer"><div style="font-size:48px;margin-bottom:12px">📖</div><h3>English</h3></div>');
  const grid = cards.join('');
  res.send(renderPage('Learning', '<div class="hero" style="padding:30px"><h1>Learn Anything</h1></div><div class="grid">' + grid + '</div>', null, true, l));
});
