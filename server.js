...
app.get('/', ah(async (req, res) => {
  ...
  res.send(renderPage('SSEWASSWA', '<div class="hero"><h1>Learn - Shop - Play - Earn</h1><div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap"><a href="/signup" class="btn btn-green" style="font-size:18px;padding:16px 32px">Get Started</a><a href="/demo" class="btn btn-gold" style="font-size:18px;padding:16px 32px">≡ƒô₧ Book Demo</a></div></div><div class="stats"><div class="stat-card"><div class="stat-num">50K+</div><div>Users</div></div><div class="stat-card"><div class="stat-num">500+</div><div>Schools</div></div><div class="stat-card"><div class="stat-num">10M+</div><div>Rewards</div></div></div><div class="grid"><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/learning\'"><div style="font-size:48px">≡ƒôÜ</div><h3>Learning</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/store\'"><div style="font-size:48px">≡ƒ¢Æ</div><h3>Store</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/marketplace\'"><div style="font-size:48px">≡ƒÅ¬</div><h3>Marketplace</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/videos\'"><div style="font-size:48px">≡ƒÄ¼</div><h3>Videos</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/games\'"><div style="font-size:48px">≡ƒÄ«</div><h3>Games</h3></div><div class="card" style="text-align:center;cursor:pointer" onclick="location.href=\'/learning/live\'"><div style="font-size:48px">≡ƒÄÑ</div><h3>Live Classes</h3></div></div><div class="card"><h2>Latest News</h2><div class="grid">'+news+'</div></div>', null, true));
}));
...
app.get('/learning/live', requireAuth, checkDb, ah(async (req, res) => {
  ...
  res.send(renderPage('Live Classes', '<div class="hero" style="padding:30px"><h1>Live Classes</h1>'+(['teacher','admin','super_admin'].includes(req.session.user.role)?'<a href="/learning/live/create" class="btn btn-green" style="margin-top:12px">Create</a>':'')+'</div><div class="grid">'+(c.rows.map(cl => '<div class="card"><div class="badge badge-blue" style="margin-bottom:8px">'+new Date(cl.scheduled_at).toLocaleDateString()+'</div><h3>'+esc(cl.title)+'</h3><p>'+esc(cl.subject)+' - '+esc(cl.class)+'</p><p>≡ƒæ¿ΓÇì≡ƒÅ½ '+esc(cl.teacher_name)+' | ≡ƒæÑ '+cl.students+'</p><div class="stat-num" style="font-size:20px;margin:12px 0">UGX '+cl.price+'</div><a href="/learning/live/join/'+cl.id+'" class="btn btn-green" style="width:100%">Join</a></div>').join('')||'<div class="card"><p>No classes</p></div>')+'</div>', {tenant_name:req.tenant.name}, false, req.lang));
}));
...
app.post('/demo', checkDb, ah(async (req, res) => {
  ...
  try {
    ...
    await sendWhatsApp(req.body.phone, 'Γ£à '+req.body.school_name+' is LIVE!\nTeacher Code: *'+code+'*\nLink: https://'+req.headers.host+'/school/'+sub);
...
app.post('/super-admin/broadcast', requireAuth, requireRole('super_admin'), checkDb, ah(async (req, res) => {
  ...
  res.send(renderPage('Sent', '<div class="card"><h1>Γ£à Sent to '+sent+'</h1><a href="/super-admin" class="btn">Back</a></div>', {tenant_name:req.tenant.name}));
}));
...
app.get('/debug-login', checkDb, ah(async (req, res) => {
  ...
  try {
    ...
    output += '<p class="badge badge-green">Database connected Γ£à</p>';
  } catch(e) {
  ...
  try {
    ...
    output += '<p>User exists: ' + (user.rows.length > 0 ? 'YES Γ£à' : 'NO Γ¥î') + '</p>';
    if (user.rows.length > 0) {
      ...
      output += '<p>Password "admin123" matches: ' + (match ? 'YES Γ£à' : 'NO Γ¥î') + '</p>';
    }
  } catch(e) {
  ...
  try {
    ...
    output += '<p>Wallet exists: ' + (wallet.rows.length > 0 ? 'YES Γ£à' : 'NO Γ¥î') + '</p>';
    if (wallet.rows[0]) {
...
app.post('/create-site', checkDb, ah(async (req, res) => {
  try {
    ...
    res.send(renderPage('Success','<div class="card" style="text-align:center"><h1>Γ£à Created!</h1><p>Code: '+esc(req.body.signup_code.toUpperCase())+'</p><a href="/login" class="btn">Login</a></div>',null,true));
