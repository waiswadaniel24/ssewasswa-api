...
const requireTenant = async (req, res, next) => {
  try {
    ...
    const tenantQuery = await pool.query('SELECT * FROM tenants WHERE subdomain = $1', [subdomain === 'localhost' || subdomain.includes('onrender')? 'main' : subdomain]);

    if (!tenantQuery.rows[0]) {
      // Default to main tenant for direct access
...
// === DATABASE INIT - BULLETPROOF v5.1 ===
async function initDB() {
  console.log('Starting database initialization v5.1...');

  // Force clean reset if lock exists from failed run
  try {
  ...
  try {
    ...
    const tables = [
      'settings', 'courses', 'chat_messages', 'news_cache', 'feedback_messages',
      'feedback_threads', 'comments', 'grants', 'donor_campaigns', 'donations',
      'surveys', 'wallets', 'cart_items', 'order_items', 'orders', 'market_items',
      'grades', 'attendance', 'fees', 'students', 'users', 'revenue_log', 'tenants'
    ];
    ...
    await client.query(`
      CREATE TABLE settings (
        id SERIAL PRIMARY KEY,
        tenant_id INTEGER REFERENCES tenants(id) ON DELETE CASCADE,
        site_name TEXT DEFAULT 'SSEWASSWA FOUNDATION UGANDA',
        hero_title TEXT,
        hero_subtitle TEXT,
        ...
        org_type TEXT DEFAULT 'platform',
        verified BOOLEAN DEFAULT false,
        public_profile BOOLEAN DEFAULT true,
        subscription_tier TEXT DEFAULT 'free',
        developer_name TEXT DEFAULT 'SSEWASSWA Foundation',
        developer_bio TEXT DEFAULT 'Building digital infrastructure for African education',
        developer_whatsapp TEXT DEFAULT '0789736737',
        developer_email TEXT DEFAULT 'waiswadaniel24@gmail.com',
    ...
    await client.query(`
      INSERT INTO wallets (tenant_id, user_email, balance)
      VALUES ($1, $2, $3)
      ON CONFLICT (tenant_id, user_email) DO NOTHING
    `, [tenantId, 'waiswadaniel24@gmail.com', 0]);

    await client.query(`
      INSERT INTO settings (tenant_id, subscription_tier, verified)
      VALUES ($1, $2, $3)
    ...
    await client.query(`
      ...
      VALUES ($1, 'Introduction to Computers', 'Learn computer basics for African students', 'https://www.youtube.com/embed/dQw4w9WgXcQ', 'technology')
    ...
    await client.query(`
      INSERT INTO market_items (tenant_id, title, description, price, seller_email, status, image_url, stock, category)
      VALUES ($1, 'School Uniform Set', 'Complete uniform for primary students - shirt, trousers, sweater', 50000, 'waiswadaniel24@gmail.com', 'approved', 'https://via.placeholder.com/300x300?text=Uniform', 100, 'uniform')
      ON CONFLICT DO NOTHING
    `, [tenantId]);

    await client.query(`
      INSERT INTO market_items (tenant_id, title, description, price, seller_email, status, image_url, stock, category)
      VALUES ($1, 'Exercise Books Pack', '10 exercise books - 96 pages each', 15000, 'waiswadaniel24@gmail.com', 'approved', 'https://via.placeholder.com/300x300?text=Books', 200, 'stationery')
      ON CONFLICT DO NOTHING
    `, [tenantId]);

    await client.query(`
      INSERT INTO market_items (tenant_id, title, description, price, seller_email, status, image_url, stock, category)
      VALUES ($1, 'School Bag', 'Durable backpack for students', 35000, 'waiswadaniel24@gmail.com', 'approved', 'https://via.placeholder.com/300x300?text=Bag', 50, 'accessories')
      ON CONFLICT DO NOTHING
    `, [tenantId]);
...
app.get('/', requireTenant, async (req, res) => {
  ...
  const content = `
    <div class="card">
      ...
      <p>Empowering African Education Through Technology</p>
      <br>
      <a href="/marketplace" class="btn">Shop School Supplies</a>
      <a href="/login" class="btn">School Login</a>
      <a href="/create-site" class="btn">Create Free School Site</a>
    </div>
    <div class="grid">
      <div class="card"><h3>📚 E-Learning</h3><p>Access courses</p><a href="/learn">Explore</a></div>
      <div class="card"><h3>📰 News</h3><p>Education news</p><a href="/news">Read</a></div>
      <div class="card"><h3>💬 Community</h3><p>Join discussions</p><a href="/comments">Join</a></div>
      <div class="card"><h3>🏆 Rankings</h3><p>Top schools</p><a href="/ranking">View</a></div>
    </div>
  `;
...
app.get('/news', requireFeature('news'), async (req, res) => {
  try {
    ...
    const content = `
      <div class="card"><h1>Education News</h1></div>
      ${items.map(item => `
        <div class="card">
          <h3>${item.title}</h3>
          <p>${item.contentSnippet}</p>
          <small>${new Date(item.pubDate).toLocaleDateString()}</small><br>
...
app.get('/learn', requireFeature('learn'), requireTenant, async (req, res) => {
  ...
  const content = `
    <div class="card"><h1>E-Learning Courses</h1></div>
    <div class="grid">
      ${courses.rows.map(course => `
        <div class="card">
          ...
          <a href="${course.video_url}" target="_blank" class="btn">Watch Course</a>
        </div>
      `).join('')}
    </div>
  `;
...
app.get('/kids', requireFeature('kids'), (req, res) => {
  const content = `<div class="card"><h1>Kids Zone</h1><p>Educational games and activities for children</p></div>`;
  res.send(renderPage('Kids', content, req));
});

app.get('/entertainment', requireFeature('entertainment'), (req, res) => {
  const content = `<div class="card"><h1>Entertainment Hub</h1><p>Stories, music, and videos for students</p></div>`;
  res.send(renderPage('Entertainment', content, req));
});
...
app.get('/ranking', requireFeature('ranking'), async (req, res) => {
  ...
  const content = `
    <div class="card"><h1>School Rankings</h1></div>
    <div class="card">
      <table>
        <tr><th>Rank</th><th>School</th><th>Score</th></tr>
        ${schools.rows.map((s, i) => `<tr><td>${i+1}</td><td>${s.name}</td><td>${s.ranking_score}</td></tr>`).join('')}
      </table>
    </div>
  `;
  ...
  const content = `
    <div class="card">
      ...
      <form method="POST" action="/comments/post">
        ...
        <input name="topic" placeholder="Topic (optional)" value="general">
        <button class="btn">Post Comment</button>
      </form>
    </div>
    ${comments.rows.map(c => `
      <div class="card">
        <strong>${c.user_name}</strong> <small>${new Date(c.created_at).toLocaleString()}</small>
        <p>${c.comment_text}</p>
        <small>Topic: ${c.topic}</small>
      </div>
    `).join('')}
  `;
...
app.get('/chat', requireFeature('chat'), (req, res) => {
  const content = `
    <div class="card"><h1>Live Chat Room</h1></div>
    <div class="card">
      <div id="messages" style="height: 300px; overflow-y: auto; border: 1px solid #ddd; padding: 1rem; margin-bottom: 1rem;"></div>
      <input id="username" placeholder="Your Name" style="width: 30%;">
      <input id="message" placeholder="Type message..." style="width: 50%;">
    ...
    <script>
      ...
      socket.on('new_message', (data) => {
        document.getElementById('messages').innerHTML += \`<p><strong>\${data.user_name}:</strong> \${data.message} <small>\${data.time}</small></p>\`;
      });
      function sendMessage() {
        const user_name = document.getElementById('username').value || 'Anonymous';
        const message = document.getElementById('message').value;
        if (message) {
          socket.emit('send_message', { room, user_name, message });
...
app.get('/marketplace', requireFeature('marketplace'), requireTenant, async (req, res) => {
  ...
  const content = `
    <div class="card">
      ...
      <p>Buy uniforms, books, and school supplies</p>
    </div>
    <div class="grid">
      ${items.rows.map(item => `
        <div class="card item-card">
          ...
          <p><strong>UGX ${item.price.toLocaleString()}</strong></p>
          <p>Stock: ${item.stock}</p>
          <form method="POST" action="/marketplace/cart/add" style="display:inline;">
            <input type="hidden" name="item_id" value="${item.id}">
            <input type="number" name="quantity" value="1" min="1" max="${item.stock}" style="width:60px;">
            <button class="btn">${t('add_to_cart', req.session.lang)}</button>
          </form>
        </div>
      `).join('')}
    </div>
    <div class="card"><a href="/marketplace/cart" class="btn">View Cart</a></div>
  `;
...
app.post('/marketplace/cart/add', requireAuth, async (req, res) => {
  ...
  `, [req.session.user.email, item_id, quantity || 1]);
  res.redirect('/marketplace/cart');
});

app.get('/marketplace/cart', requireAuth, async (req, res) => {
  const cart = await pool.query(`
    SELECT c.id, c.quantity, m.title, m.price, m.image_url, m.id as item_id
    FROM cart_items c
    JOIN market_items m ON c.item_id = m.id
  ...
  const content = `
    <div class="card"><h1>Your Shopping Cart</h1></div>
    ${cart.rows.length === 0? '<div class="card">Your cart is empty</div>' : `
      <div class="card">
        <table>
          ...
          `).join('')}
          <tr><td colspan="3"><strong>Total</strong></td><td><strong>UGX ${total.toLocaleString()}</strong></td></tr>
        </table>
        <br>
        <form method="POST" action="/marketplace/checkout">
          ...
          <input name="momo_number" placeholder="MTN/Airtel Number (if Mobile Money)">
          <input name="delivery_address" placeholder="Delivery Address" required>
          <button class="btn">Checkout - UGX ${total.toLocaleString()}</button>
        </form>
      </div>
    `}
    <div class="card"><a href="/marketplace">Continue Shopping</a></div>
  `;
...
app.post('/marketplace/checkout', requireAuth, requireTenant, async (req, res) => {
  ...
  const cart = await pool.query('SELECT c.*, m.price FROM cart_items c JOIN market_items m ON c// === CONTINUED FROM PART 1 ===
    JOIN market_items m ON c.item_id = m.id WHERE c.user_email = $1
  `, [req.session.user.email]);
  ...
  try {
    ...
    const order = await client.query(`
      INSERT INTO orders (tenant_id, user_email, total_amount, payment_method, momo_number, delivery_address, status)
      VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id
    `, [req.tenant.id, req.session.user.email, total, payment_method, momo_number, delivery_address, payment_method === 'cash'? 'pending' : 'paid']);

    for (const item of cart.rows) {
      await client.query(`
        INSERT INTO order_items (order_id, item_id, quantity, price)
        VALUES ($1, $2, $3, $4)
      `, [order.rows[0].id, item.item_id, item.quantity, item.price]);
    ...
    // Log revenue
    await client.query(`
      INSERT INTO revenue_log (type, gross_amount, commission, tenant_id, description)
      VALUES ($1, $2, $3, $4, $5)
    `, ['marketplace', total, total * 0.05, req.tenant.id, `Order #${order.rows[0].id}`]);
    ...
    const content = `
      <div class="card">
        ...
        <p>Order ID: #${order.rows[0].id}</p>
        <p>Total: UGX ${total.toLocaleString()}</p>
        <p>Payment: ${payment_method === 'momo'? 'Mobile Money - ' + momo_number : 'Cash on Delivery'}</p>
        <p>Delivery: ${delivery_address}</p>
        <a href="/marketplace" class="btn">Continue Shopping</a>
...
app.get('/orders', requireAuth, async (req, res) => {
  ...
  const content = `
    <div class="card"><h1>My Orders</h1></div>
    ${orders.rows.length === 0? '<div class="card">No orders yet</div>' : orders.rows.map(order => `
      <div class="card">
        <h3>Order #${order.id}</h3>
        <p>Total: UGX ${order.total_amount.toLocaleString()}</p>
...
app.get('/donor-portal', requireTenant, requireFeature('donations'), async (req, res) => {
  ...
  const content = `
    <div class="card"><h1>Donor Portal</h1><p>Support education initiatives</p></div>
    <div class="grid">
...
app.get('/api', requireTenant, requireFeature('api_access'), (req, res) => {
  const content = `<div class="card"><h1>API Access</h1><p>Your API Key: <code>ssewasswa_${req.tenant.subdomain}_${req.tenant.id}</code></p></div>`;
...
app.get('/advertising', requireTenant, requireFeature('advertising'), (req, res) => {
  const content = `<div class="card"><h1>Advertising</h1><p>Promote your school or business on our platform</p></div>`;
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!user.rows[0]) return res.status(401).send(renderPage('Login', '<div class="card"><h1>Error</h1><p>Invalid credentials</p></div>', req));

    const valid = await bcrypt.compare(password, user.rows[0].password_hash);
    if (!valid) return res.status(401).send(renderPage('Login', '<div class="card"><h1>Error</h1><p>Invalid credentials</p></div>', req));
...
app.post('/create-site', async (req, res) => {
  ...
  try {
    ...
    res.send(renderPage('Success', `<div class="card"><h1>Site Created!</h1><p>Your school site: http://${subdomain}.ssewasswa.org</p></div>`, req));
  } catch (e) {
  ...
  const content = `
    ...
    <div class="card">
      <table>
        ...
        ${students.rows.map(s => `<tr><td>${s.name}</td><td>${s.class}</td><td>${s.guardian_name}</td><td>UGX ${s.balance}</td></tr>`).join('')}
      </table>
    </div>
  `;
    ...
    <div class="card">
      ...
      <form method="POST" action="/app/fees/add">
        <select name="student_id" required>
          ...
          ${students.rows.map(s => `<option value="${s.id}">${s.name}</option>`).join('')}
        </select>
        <input name="amount" type="number" placeholder="Amount" required>
      ...
      <table>
        ...
        ${fees.rows.map(f => `
          <tr>
            ...
            <td>UGX ${f.paid.toLocaleString()}</td>
            <td>UGX ${(f.amount - f.paid).toLocaleString()}</td>
            <td>${f.term} ${f.year}</td>
            <td>
              <form method="POST" action="/app/fees/pay/${f.id}" style="display:inline;">
                <input type="number" name="amount" placeholder="Pay" style="width:80px;">
...
app.get('/app/attendance', requireAuth, async (req, res) => {
  ...
  `, [req.session.user.tenant_id, today]);
  ...
  const content = `
    ...
    <div class="card">
      <table>
        <tr><th>Student</th><th>Status</th><th>Time</th></tr>
        ${attendance.rows.map(a => `<tr><td>${a.name}</td><td>${a.status}</td><td>${new Date(a.created_at).toLocaleTimeString()}</td></tr>`).join('')}
      </table>
    </div>
  `;
...
app.post('/app/attendance/mark', requireAuth, async (req, res) => {
  ...
  `, [req.session.user.tenant_id, student_id, status]);
...
app.get('/app/grades', requireAuth, async (req, res) => {
  ...
  const content = `
    ...
    <div class="card">
      <table>
        <tr><th>Student</th><th>Subject</th><th>Score</th><th>Term</th><th>Year</th></tr>
        ${grades.rows.map(g => `<tr><td>${g.name}</td><td>${g.subject}</td><td>${g.score}%</td><td>${g.term}</td><td>${g.year}</td></tr>`).join('')}
      </table>
    </div>
  `;
...
app.get('/super-admin', requireAuth, requireRole('super_admin'), (req, res) => {
  const content = `
    <div class="card"><h1>Super Admin Dashboard</h1></div>
    <div class="grid">
      <div class="card"><h3>🏫 Tenants</h3><a href="/super-admin/tenants" class="btn">Manage</a></div>
      <div class="card"><h3>👤 Users</h3><a href="/super-admin/users" class="btn">Manage</a></div>
      <div class="card"><h3>💬 Comments</h3><a href="/super-admin/comments" class="btn">Moderate</a></div>
      <div class="card"><h3>📧 Feedback</h3><a href="/super-admin/feedback" class="btn">View</a></div>
      <div class="card"><h3>⚙️ Features</h3><a href="/super-admin/features" class="btn">Configure</a></div>
      <div class="card"><h3>💰 Revenue</h3><a href="/super-admin/revenue" class="btn">Track</a></div>
    </div>
  `;
...
app.get('/super-admin/tenants', requireAuth, requireRole('super_admin'), async (req, res) => {
  ...
  const content = `
    <div class="card"><h1>All Tenants</h1></div>
    <div class="card">
      <table>
        <tr><th>ID</th><th>Name</th><th>Subdomain</th><th>Plan</th><th>Score</th><th>Action</th></tr>
        ${tenants.rows.map(t => `
          <tr>
            <td>${t.id}</td>
            <td>${t.name}</td>
            ...
            <td>
              <form method="POST" action="/super-admin/verify" style="display:inline;">
...
app.get('/super-admin/users', requireAuth, requireRole('super_admin'), async (req, res) => {
  ...
  const content = `
    <div class="card"><h1>All Users</h1></div>
    <div class="card">
      <table>
        <tr><th>ID</th><th>Email</th><th>Role</th><th>Tenant</th><th>Created</th></tr>
        ${users.rows.map(u => `
          <tr>
            <td>${u.id}</td>
            <td>${u.email}</td>
            <td>${u.role}</td>
            <td>${u.tenant_name || 'N/A'}</td>
...
app.get('/super-admin/comments', requireAuth, requireRole('super_admin'), async (req, res) => {
  ...
  const content = `
    <div class="card"><h1>Moderate Comments</h1></div>
    ${comments.rows.map(c => `
      <div class="card">
        <strong>${c.user_name}</strong> | Status: ${c.status}
        <p>${c.comment_text}</p>
...
app.get('/super-admin/feedback', requireAuth, requireRole('super_admin'), async (req, res) => {
  ...
  const content = `
    <div class="card"><h1>Feedback Threads</h1></div>
    ${threads.rows.map(t => `
      <div class="card">
        <h3>${t.subject}</h3>
        <p>From: ${t.user_name} (${t.user_email})</p>
        <p>Status: ${t.status}</p>
...
app.get('/super-admin/features', requireAuth, requireRole('super_admin'), (req, res) => {
  const content = `
    <div class="card"><h1>Feature Flags</h1></div>
    <div class="card">
...
app.get('/super-admin/revenue', requireAuth, requireRole('super_admin'), async (req, res) => {
  ...
  const content = `
    <div class="card">
      ...
      <h2>Total: UGX ${total.rows[0].total?.toLocaleString() || 0}</h2>
    </div>
    <div class="card">
      <table>
        <tr><th>Type</th><th>Gross</th><th>Commission</th><th>Description</th><th>Date</th></tr>
        ${revenue.rows.map(r => `
          <tr>
            <td>${r.type}</td>
            <td>UGX ${r.gross_amount.toLocaleString()}</td>
            <td>UGX ${r.commission.toLocaleString()}</td>
            <td>${r.description}</td>
...
app.get('/about-developer', (req, res) => {
  const content = `
    <div class="card">
      ...
      <p><strong>SSEWASSWA Foundation</strong></p>
      <p>Building digital infrastructure for African education</p>
      <p>WhatsApp: 0789736737</p>
      <p>Email: waiswadaniel24@gmail.com</p>
...
// === SOCKET.IO CHAT - SINGLE INSTANCE ===
