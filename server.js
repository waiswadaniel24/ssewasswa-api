...
app.get('/admin/staff/payroll', requireLogin, requireRole(['admin']), async (req, res) => {
  try {
    ...
    const monthOptions = months.map(m => '<option value="' + m + '"' + (m === currentMonth ? ' selected' : '') + '>' + m + '</option>').join('');
    
    const staffRows = staff.rows.map(s => {
      const isPaid = paidIds.includes(s.id);
      const status = isPaid ? 'Paid' : 'Pending';
      const statusClass = isPaid ? 'paid' : 'unpaid';
      const action = isPaid 
        ? '<span style="color:green">Paid</span>' 
        : '<a href="/admin/staff/pay/' + s.id + '?month=' + currentMonth + '&year=' + currentYear + '" class="btn btn-green" style="padding:5px 10px;font-size:12px">Pay Now</a>';
      
      return '<tr class="' + statusClass + '"><td>' + s.full_name + '</td><td>' + s.position + '</td><td>' + s.department + '</td><td>UGX ' + Number(s.monthly_salary).toLocaleString() + '</td><td>' + status + '</td><td>' + action + '</td></tr>';
    }).join('');

    const html = '<!DOCTYPE html><html><head><title>Staff Payroll</title><style>body{font-family:Arial;margin:20px;background:#f4f6f9}table{width:100%;background:white;border-collapse:collapse}th,td{padding:12px;border:1px solid #ddd;text-align:left}th{background:#2c3e50;color:white}.paid{background:#d4edda}.unpaid{background:#f8d7da}.btn{padding:8px 15px;border:none;border-radius:4px;cursor:pointer;text-decoration:none;display:inline-block}.btn-green{background:#27ae60;color:white}</style></head><body><h1>Staff Payroll - ' + currentMonth + ' ' + currentYear + '</h1><form method="GET" style="margin:20px 0"><select name="month">' + monthOptions + '</select><input type="number" name="year" value="' + currentYear + '" style="width:100px"><button type="submit">View</button></form><table><tr><th>Name</th><th>Position</th><th>Department</th><th>Salary</th><th>Status</th><th>Action</th></tr>' + staffRows + '</table><br><a href="/admin/dashboard">Back to Dashboard</a></body></html>';
