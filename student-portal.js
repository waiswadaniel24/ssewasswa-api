...
module.exports = function studentPortal(app, db, pool, renderPage, esc) {
  ...
  // ============================================================
  // NEW DATABASE MIGRATIONS — student_messages, school_announcements
  // ============================================================
  (async () => {
    const c = await pool.connect().catch(() => null);
    if (!c) return;
    try {
      await c.query(`CREATE TABLE IF NOT EXISTS student_messages (
        id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL DEFAULT 0,
        from_email VARCHAR(255) NOT NULL, to_email VARCHAR(255) NOT NULL,
        subject VARCHAR(500) NOT NULL, body TEXT NOT NULL,
        is_read BOOLEAN DEFAULT false, parent_message_id INTEGER,
        created_at TIMESTAMPTZ DEFAULT NOW()
      )`);
      await c.query(`CREATE INDEX IF NOT EXISTS idx_smsg_tenant ON student_messages(tenant_id)`);
      await c.query(`CREATE INDEX IF NOT EXISTS idx_smsg_to ON student_messages(to_email)`);
      await c.query(`CREATE INDEX IF NOT EXISTS idx_smsg_parent ON student_messages(parent_message_id)`);
      await c.query(`CREATE TABLE IF NOT EXISTS school_announcements (
        id SERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL DEFAULT 0,
        title VARCHAR(500) NOT NULL, body TEXT,
        priority VARCHAR(20) DEFAULT 'normal',
        author_email VARCHAR(255), is_active BOOLEAN DEFAULT true,
        created_at TIMESTAMPTZ DEFAULT NOW()
      )`);
      await c.query(`CREATE INDEX IF NOT EXISTS idx_announce_tenant ON school_announcements(tenant_id)`);
      console.log('[StudentPortal] New migrations applied');
    } catch (e) { console.error('[StudentPortal] New migration error:', e.message); }
    finally { c.release(); }
  })();
  ...
  // GET /portal/messages — Inbox with thread list
  app.get('/portal/messages', requireAuth, ah(async (req, res) => {
    const user = req.session.user, tid = user.tenant_id;
    try {
      const threads = (await pool.query(
        `SELECT DISTINCT ON (COALESCE(m.parent_message_id, m.id))
          m.*, COALESCE(s_from.first_name || ' ' || s_from.last_name, m.from_email) as from_name,
          (SELECT COUNT(*) FROM student_messages replies WHERE replies.parent_message_id = COALESCE(m.parent_message_id, m.id) AND replies.created_at > m.created_at) as reply_count
        FROM student_messages m
        LEFT JOIN students s_from ON s_from.email = m.from_email AND s_from.tenant_id = m.tenant_id
        WHERE m.tenant_id = $1 AND (m.to_email = $2 OR m.from_email = $2)
        ORDER BY COALESCE(m.parent_message_id, m.id), m.created_at DESC`,
        [tid, user.email]
      )).rows;

      // Mark incoming messages as read
      try {
        await pool.query(`UPDATE student_messages SET is_read = true WHERE tenant_id=$1 AND to_email=$2 AND is_read=false`, [tid, user.email]);
      } catch(e){}

      const threadsHtml = threads.map(t => {
        const unread = !t.is_read && t.from_email !== user.email;
        return `<a href="/portal/messages/${t.id}" style="display:block;padding:14px 16px;border-bottom:1px solid #f1f5f9;text-decoration:none;${unread ? 'background:#f0f4ff;' : ''}transition:.15s" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='${unread ? '#f0f4ff' : 'transparent'}'">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <div style="font-size:14px;font-weight:${unread ? '700' : '500'};color:#1e293b">${esc(t.subject)}</div>
            <div style="font-size:11px;color:#94a3b8">${fmtDate(t.created_at)}</div>
          </div>
          <div style="font-size:12px;color:#64748b;margin-top:4px">From: ${esc(t.from_name || t.from_email)}</div>
          ${t.reply_count > 0 ? `<div style="font-size:11px;color:#4f46e5;margin-top:2px">${t.reply_count} replies</div>` : ''}
          ${unread ? '<div style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#4f46e5;margin-top:4px"></div>' : ''}
        </a>`;
      }).join('');
      ...
      const html = SP_CSS + `<div style="max-width:800px;margin:0 auto">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
          <h1 style="font-size:24px;color:#1e293b">💬 Messages</h1>
          <a href="/portal/messages" class="sp-btn sp-btn-primary" style="font-size:12px">✉️ Compose</a>
        </div>
        <div class="sp-card" style="padding:0;overflow:hidden">
          <div style="padding:16px 20px;border-bottom:2px solid #e2e8f0;font-size:13px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.5px">Inbox</div>
          <div style="max-height:500px;overflow-y:auto">
            ${threadsHtml || '<div style="padding:40px;text-align:center;color:#94a3b8">No messages yet</div>'}
          </div>
        </div>
      </div>`;
      res.send(renderPage('Messages', html, user, req));
    } catch(e) {
      req.session.flash = { type: 'error', msg: 'Failed to load messages' };
      res.redirect('/portal');
    }
  }));

  // POST /portal/messages — Send a new message
  app.post('/portal/messages', requireAuth, ah(async (req, res) => {
    const user = req.session.user, tid = user.tenant_id;
    ...
    try {
      await pool.query(
        `INSERT INTO student_messages (tenant_id, from_email, to_email, subject, body) VALUES ($1,$2,$3,$4,$5)`,
        [tid, user.email, to_email.trim(), subject.trim(), body.trim()]
      );
      req.session.flash = { type: 'success', msg: 'Message sent!' };
    } catch(e) {
      req.session.flash = { type: 'error', msg: 'Failed to send message' };
    }
    res.redirect('/portal/messages');
  }));

  // GET /portal/messages/:id — Thread view
  app.get('/portal/messages/:id', requireAuth, ah(async (req, res) => {
    const user = req.session.user, tid = user.tenant_id, msgId = req.params.id;
    try {
      const root = (await pool.query(`SELECT * FROM student_messages WHERE id=$1 AND tenant_id=$2`, [msgId, tid])).rows[0];
      if (!root) return res.redirect('/portal/messages');

      const threadId = root.parent_message_id || root.id;
      const messages = (await pool.query(
        `SELECT m.*, COALESCE(s.first_name || ' ' || s.last_name, m.from_email) as from_name
         FROM student_messages m
         LEFT JOIN students s ON s.email = m.from_email AND s.tenant_id = m.tenant_id
         WHERE (m.id = $1 OR m.parent_message_id = $1) AND m.tenant_id = $2
         ORDER BY m.created_at ASC`,
        [threadId, tid]
      )).rows;

      // Mark all as read
      try { await pool.query(`UPDATE student_messages SET is_read=true WHERE tenant_id=$1 AND (id=$2 OR parent_message_id=$2) AND to_email=$3`, [tid, threadId, user.email]); } catch(e){}

      const messagesHtml = messages.map(m => {
        const isMe = m.from_email === user.email;
        return `<div style="display:flex;gap:12px;margin-bottom:16px;${isMe ? 'flex-direction:row-reverse' : ''}">
          <div style="width:36px;height:36px;border-radius:50%;background:${isMe ? '#4f46e5' : '#e2e8f0'};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:${isMe ? '#fff' : '#475569'};flex-shrink:0">${esc((m.from_name || '?').charAt(0).toUpperCase())}</div>
          <div style="max-width:70%">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;${isMe ? 'flex-direction:row-reverse' : ''}">
              <span style="font-size:12px;font-weight:600;color:#1e293b">${esc(m.from_name || m.from_email)}</span>
              <span style="font-size:11px;color:#94a3b8">${fmtDateTime(m.created_at)}</span>
            </div>
            <div style="background:${isMe ? '#eef2ff' : '#f8fafc'};padding:12px 16px;border-radius:12px;font-size:13px;color:#1e293b;line-height:1.6;border:1px solid ${isMe ? '#c7d2fe' : '#f1f5f9'}">${esc(m.body)}</div>
          </div>
        </div>`;
      }).join('');
      ...
      const html = SP_CSS + `<div style="max-width:800px;margin:0 auto">
        ...
        <div style="margin-bottom:20px">
          <a href="/portal/messages" style="color:#64748b;font-size:14px;text-decoration:none">← Back to Inbox</a>
        </div>
        <div class="sp-card" style="padding:24px;margin-bottom:16px">
          <h2 style="font-size:18px;color:#1e293b;margin-bottom:4px">${esc(root.subject)}</h2>
          <p style="font-size:12px;color:#94a3b8">${messages.length} message(s)</p>
        </div>
        <div class="sp-card" style="padding:20px;margin-bottom:16px;max-height:400px;overflow-y:auto">
          ${messagesHtml}
        </div>
        <div class="sp-card" style="padding:20px">
          <h3 style="font-size:14px;color:#1e293b;margin-bottom:12px">Reply</h3>
          <form method="POST" action="/portal/messages/${esc(root.id)}/reply" style="display:flex;flex-direction:column;gap:10px">
            <textarea name="body" required rows="3" placeholder="Type your reply..." style="width:100%;padding:10px 14px;border:2px solid #e2e8f0;border-radius:10px;font-size:13px;resize:vertical"></textarea>
            <button type="submit" class="sp-btn sp-btn-primary" style="align-self:flex-end">📤 Send Reply</button>
          </form>
        </div>
      </div>`;
      res.send(renderPage(root.subject, html, user, req));
    } catch(e) {
      req.session.flash = { type: 'error', msg: 'Failed to load thread' };
      res.redirect('/portal/messages');
    }
  }));

  // POST /portal/messages/:id/reply — Reply to thread
  app.post('/portal/messages/:id/reply', requireAuth, ah(async (req, res) => {
    const user = req.session.user, tid = user.tenant_id, msgId = req.params.id;
    const { body } = req.body;
    if (!body || !body.trim()) {
      req.session.flash = { type: 'error', msg: 'Reply cannot be empty' };
      return res.redirect(`/portal/messages/${msgId}`);
    }
    try {
      const root = (await pool.query(`SELECT * FROM student_messages WHERE id=$1 AND tenant_id=$2`, [msgId, tid])).rows[0];
      if (!root) return res.redirect('/portal/messages');
      const threadId = root.parent_message_id || root.id;
      const toEmail = root.from_email === user.email ? root.to_email : root.from_email;
      await pool.query(
        `INSERT INTO student_messages (tenant_id, from_email, to_email, subject, body, parent_message_id) VALUES ($1,$2,$3,$4,$5,$6)`,
        [tid, user.email, toEmail, 'Re: ' + root.subject, body.trim(), threadId]
      );
      req.session.flash = { type: 'success', msg: 'Reply sent!' };
    } catch(e) {
      req.session.flash = { type: 'error', msg: 'Failed to send reply' };
    }
    res.redirect(`/portal/messages/${msgId}`);
  }));
  ...
  // GET /portal/progress-report — Term summary
  app.get('/portal/progress-report', requireAuth, ah(async (req, res) => {
    const user = req.session.user, tid = user.tenant_id;
    const student = await resolveStudent(user);
    if (!student) return res.redirect('/portal');
    ...
    try {
      grades = (await pool.query(
        `SELECT m.*, s.name as subject_name FROM marks m LEFT JOIN subjects s ON s.id=m.subject_id WHERE m.tenant_id=$1 AND m.student_id=$2 AND (m.term=$3 OR m.term IS NULL) ORDER BY m.created_at DESC`,
        [tid, student.id, term]
      )).rows;
    } catch(e) {
      try {
        grades = (await pool.query(
          `SELECT g.*, sub.name as subject_name FROM grades g LEFT JOIN subjects sub ON sub.id=g.subject_id WHERE g.tenant_id=$1 AND g.student_id=$2 AND (g.term=$3 OR g.term IS NULL) ORDER BY g.created_at DESC`,
          [tid, student.id, term]
        )).rows;
      } catch(e2){}
    }

    try { attendance = (await pool.query(`SELECT * FROM attendance WHERE tenant_id=$1 AND student_id=$2 ORDER BY date DESC LIMIT 60`, [tid, student.id])).rows; } catch(e){
      try { attendance = (await pool.query(`SELECT * FROM attendance_records WHERE tenant_id=$1 AND person_id=$2 ORDER BY date DESC LIMIT 60`, [tid, student.id])).rows; } catch(e2){}
    }

    const avgGrade = grades.length > 0 ? Math.round(grades.reduce((s, g) => s + Number(g.grade || g.score || 0), 0) / grades.length) : 0;
    const highest = grades.length > 0 ? Math.max(...grades.map(g => Number(g.grade || g.score || 0))) : 0;
    const lowest = grades.length > 0 ? Math.min(...grades.map(g => Number(g.grade || g.score || 0))) : 0;
    ...
    const gradesHtml = grades.slice(0, 15).map(g => {
      const score = Number(g.grade || g.score || 0);
      return `<tr>
        <td><strong>${esc(g.subject_name || g.subject || '—')}</strong></td>
        <td>${esc(g.exam_type || g.exam_name || '—')}</td>
        <td><span style="font-weight:700;color:${gradeColor(score)}">${score}%</span></td>
        <td>${fmtDate(g.created_at)}</td>
      </tr>`;
    }).join('');
    ...
    const html = SP_CSS + `<div style="max-width:1000px;margin:0 auto">
      ${nav('')}
      <h1 style="font-size:24px;color:#1e293b;margin-bottom:20px">📊 Progress Report — ${esc(term)}</h1>
      <div class="sp-card" style="padding:24px;margin-bottom:16px;text-align:center;border:2px solid #e2e8f0">
        <h2 style="margin:0 0 4px;color:#1e293b">${esc(student.first_name || '')} ${esc(student.last_name || '')}</h2>
        <p style="margin:0;color:#94a3b8;font-size:13px">${esc(student.class_name || '')} · ${esc(student.admission_number || '')}</p>
      </div>
      <div class="stats" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:14px;margin-bottom:20px">
        <div class="stat-card"><div class="stat-num" style="color:#4f46e5">${avgGrade}%</div><div class="muted" style="font-size:11px">Average Grade</div></div>
        <div class="stat-card"><div class="stat-num" style="color:#16a34a">${highest}%</div><div class="muted" style="font-size:11px">Highest Score</div></div>
        <div class="stat-card"><div class="stat-num" style="color:#dc2626">${lowest}%</div><div class="muted" style="font-size:11px">Lowest Score</div></div>
        <div class="stat-card"><div class="stat-num" style="color:#059669">${attRate}%</div><div class="muted" style="font-size:11px">Attendance Rate</div></div>
        <div class="stat-card"><div class="stat-num" style="color:#4f46e5">${present}</div><div class="muted" style="font-size:11px">Days Present</div></div>
        <div class="stat-card"><div class="stat-num" style="color:#dc2626">${absent}</div><div class="muted" style="font-size:11px">Days Absent</div></div>
      </div>
      <div class="card" style="padding:20px">
        <h3 style="font-size:15px;color:#1e293b;margin-bottom:14px">📋 Subject Grades</h3>
        <div style="overflow-x:auto"><table class="sp-table">
          <thead><tr><th>Subject</th><th>Exam Type</th><th>Score</th><th>Date</th></tr></thead>
          <tbody>${gradesHtml || '<tr><td colspan="4" style="text-align:center;color:#94a3b8;padding:30px">No grades available for this term</td></tr>'}</tbody>
        </table></div>
      </div>
    </div>`;
    res.send(renderPage('Progress Report', html, user, req));
  }));
  ...
  // GET /portal/announcements — List announcements
  app.get('/portal/announcements', requireAuth, ah(async (req, res) => {
    const user = req.session.user, tid = user.tenant_id;
    try {
      const announcements = (await pool.query(
        `SELECT * FROM school_announcements WHERE tenant_id=$1 AND is_active=true ORDER BY priority DESC, created_at DESC LIMIT 20`,
        [tid]
      )).rows;

      const annHtml = announcements.map(a => {
        const priorityColors = { urgent: '#dc2626', high: '#f59e0b', normal: '#64748b' };
        const pColor = priorityColors[a.priority] || '#64748b';
        return `<div class="sp-card" style="margin-bottom:12px;border-left:4px solid ${pColor}">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
            <h3 style="margin:0;font-size:15px;color:#1e293b">${esc(a.title)}</h3>
            <div style="display:flex;align-items:center;gap:8px">
              ${a.priority && a.priority !== 'normal' ? `<span style="font-size:10px;font-weight:700;text-transform:uppercase;padding:3px 8px;border-radius:6px;background:${pColor}20;color:${pColor}">${esc(a.priority)}</span>` : ''}
              <span style="font-size:11px;color:#94a3b8">${fmtDate(a.created_at)}</span>
            </div>
          </div>
          <p style="font-size:13px;color:#475569;line-height:1.6;margin:0">${esc(a.body || '')}</p>
          ${a.author_email ? `<div style="font-size:11px;color:#94a3b8;margin-top:8px">Posted by ${esc(a.author_email)}</div>` : ''}
        </div>`;
      }).join('');

      const html = SP_CSS + `<div style="max-width:900px;margin:0 auto">
        ${nav('')}
        <h1 style="font-size:24px;color:#1e293b;margin-bottom:20px">📢 Announcements</h1>
        ${annHtml || '<div class="sp-card" style="text-align:center;padding:40px;color:#94a3b8">No announcements at this time</div>'}
      </div>`;
      res.send(renderPage('Announcements', html, user, req));
    } catch(e) {
      req.session.flash = { type: 'error', msg: 'Failed to load announcements' };
      res.redirect('/portal');
    }
  }));

  // POST /admin/announcements — Create announcement (admin only)
  app.post('/admin/announcements', requireAuth, ah(async (req, res) => {
    const user = req.session.user, tid = user.tenant_id;
    ...
    try {
      await pool.query(
        `INSERT INTO school_announcements (tenant_id, title, body, priority, author_email) VALUES ($1,$2,$3,$4,$5)`,
        [tid, title.trim(), body.trim(), (priority || 'normal').trim(), user.email]
      );
      res.json({ success: true, msg: 'Announcement created' });
    } catch(e) {
  ...
  // POST /portal/fees/pay — Initiate mobile money payment
  app.post('/portal/fees/pay', requireAuth, ah(async (req, res) => {
    const user = req.session.user, tid = user.tenant_id;
    const student = await resolveStudent(user);
    if (!student) return res.json({ error: 'Student profile not found' });

    const { amount, phone_number, fee_id } = req.body;
    const payAmount = Number(amount);
    if (!payAmount || payAmount <= 0 || !phone_number) {
      return res.json({ error: 'Valid amount and phone number are required' });
    }

    try {
      // Record the payment attempt
      const ref = 'MM-' + genToken().substring(0, 12).toUpperCase();
      const payment = (await pool.query(
        `INSERT INTO payments (tenant_id, student_id, amount, method, reference, status) VALUES ($1,$2,$3,'Mobile Money',$4,'pending') RETURNING id`,
        [tid, student.id, payAmount, ref]
      )).rows[0];

      // Track revenue if available
      try {
        if (typeof global.trackRevenue === 'function') {
          global.trackRevenue({ tenant_id: tid, amount: payAmount, source: 'student_fee_payment', reference: ref });
        }
      } catch(e){}

      // Simulate mobile money initiation (in production, integrate with actual provider)
      res.json({
        success: true,
        payment_id: payment.id,
        reference: ref,
        amount: payAmount,
        message: `Payment of ${fmtMoney(payAmount)} initiated via Mobile Money. Reference: ${ref}. You will receive an STC prompt on ${esc(phone_number)}.`
      });
    } catch(e) {
  ...
  // ============================================================
  // DARK MODE CSS — Student Portal
  // ============================================================
  const SP_DARK_CSS = `<style>
    @media(prefers-color-scheme:dark){
      .sp-nav a{background:#1e293b;color:#cbd5e1}
      .sp-nav a:hover{background:#334155}
      .sp-nav a.active{background:#4f46e5;color:#fff}
      .sp-btn-secondary{background:#1e293b;color:#cbd5e1}
      .sp-btn-danger{background:#451a1a;color:#fca5a5}
      .sp-card{background:#1e293b;border-color:#334155}
      .sp-card:hover{box-shadow:0 4px 16px rgba(0,0,0,.3)}
      .sp-info-row{border-bottom-color:#334155}
      .sp-info-label{color:#94a3b8}
      .sp-info-value{color:#f1f5f9}
      .sp-table th{background:#0f172a;color:#94a3b8;border-bottom-color:#334155}
      .sp-table td{color:#e2e8f0;border-bottom-color:#1e293b}
      .sp-table tr:hover{background:#1e293b}
      .sp-timetable .time-slot{background:#0f172a;color:#94a3b8}
      .sp-timetable .day-header{background:#4f46e5;color:#fff}
      .sp-timetable .cell{background:#1e293b;border-color:#334155}
      .sp-timetable .cell.filled{background:#1e1b4b;border-color:#312e81}
      .sp-timetable .cell.filled .subj{color:#a5b4fc}
      .sp-timetable .cell.filled .room{color:#94a3b8}
      .sp-form-grid input,.sp-form-grid select,.sp-form-grid textarea{background:#0f172a;border-color:#334155;color:#e2e8f0}
  ...
  // Inject dark mode CSS by overriding SP_CSS
  const _origSP_CSS = SP_CSS;
  // Dark mode is appended globally via a late-injected <style> block
  // We add it to the nav helper so every portal page includes it
  app.get('/portal/dark-mode.css', requireAuth, (req, res) => {
    res.type('text/css').send(SP_DARK_CSS);
  });
